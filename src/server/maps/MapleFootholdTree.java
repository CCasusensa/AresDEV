...
public class MapleFootholdTree {
	...
	public void insert(MapleFoothold f) {
		...
		if ((this.depth == maxDepth)
				|| ((f.getX1() >= this.p1.x) && (f.getX2() <= this.p2.x)
						&& (f.getY1() >= this.p1.y) && (f.getY2() <= this.p2.y))) {
			this.footholds.add(f);
		} else {
			if (this.nw == null) {
				this.nw = new MapleFootholdTree(this.p1, this.center,
						this.depth + 1);
				this.ne = new MapleFootholdTree(new Point(this.center.x,
						this.p1.y), new Point(this.p2.x, this.center.y),
						this.depth + 1);
				this.sw = new MapleFootholdTree(new Point(this.p1.x,
						this.center.y), new Point(this.center.x, this.p2.y),
						this.depth + 1);
				this.se = new MapleFootholdTree(this.center, this.p2,
						this.depth + 1);
			}
			if ((f.getX2() <= this.center.x) && (f.getY2() <= this.center.y)) {
				this.nw.insert(f);
			} else if ((f.getX1() > this.center.x)
					&& (f.getY2() <= this.center.y)) {
				this.ne.insert(f);
			} else if ((f.getX2() <= this.center.x)
					&& (f.getY1() > this.center.y)) {
				this.sw.insert(f);
			} else {
				this.se.insert(f);
			}
	...
	private MapleFoothold findWallR(Point p1, Point p2) {
		...
		if (this.nw != null) {
			...
			if (((p1.x > this.center.x) || (p2.x > this.center.x))
					&& (p1.y <= this.center.y)) {
