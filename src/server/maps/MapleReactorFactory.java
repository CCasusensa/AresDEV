...
public class MapleReactorFactory {
	...
	public static MapleReactorStats getReactor(int rid) {
		...
		if (stats == null) {
			int infoId = rid;
			MapleData reactorData = data.getData(StringUtil.getLeftPaddedStr(
					Integer.toString(infoId) + ".img", '0', 11));
			final MapleData link = reactorData.getChildByPath("info/link");
			if (link != null) {
				infoId = MapleDataTool.getIntConvert("info/link", reactorData);
				stats = reactorStats.get(Integer.valueOf(infoId));
			}
			final MapleData activateOnTouch = reactorData
					.getChildByPath("info/activateByTouch");
			boolean loadArea = false;
			if (activateOnTouch != null) {
				loadArea = MapleDataTool.getInt("info/activateByTouch",
						reactorData, 0) != 0;
			}
			if (stats == null) {
				...
				if (reactorInfoData != null) {
					...
					while (reactorInfoData != null) {
						final MapleData eventData = reactorInfoData
								.getChildByPath("event");
						if (eventData != null) {
							for (final MapleData fknexon : eventData
									.getChildren()) {
								if (fknexon.getName().equals("timeOut")) {
									continue;
								}
								Pair<Integer, Integer> reactItem = null;
								final int type = MapleDataTool.getIntConvert(
										"type", fknexon);
								if (type == 100) { // reactor waits for item
									reactItem = new Pair<Integer, Integer>(
											MapleDataTool.getIntConvert("0",
													fknexon),
											MapleDataTool.getIntConvert("1",
													fknexon));
									if (!areaSet || loadArea) { // only set area
																// of effect for
																// item-triggered
																// reactors once
										stats.setTL(MapleDataTool.getPoint(
												"lt", fknexon));
										stats.setBR(MapleDataTool.getPoint(
												"rb", fknexon));
										areaSet = true;
									}
								}
								final MapleData activeSkillID = fknexon
										.getChildByPath("activeSkillID");
								List<Integer> skillids = null;
								if (activeSkillID != null) {
									skillids = new ArrayList<Integer>();
									for (final MapleData skill : activeSkillID
											.getChildren()) {
										skillids.add(MapleDataTool
												.getInt(skill));
									}
								}
								final byte nextState = (byte) MapleDataTool
										.getIntConvert("state", fknexon);
								statedatas.add(new StateData(type, reactItem,
										skillids, nextState));
							}
							stats.addState(i, statedatas);
						}
						i++;
						reactorInfoData = reactorData.getChildByPath(Byte
								.toString(i));
