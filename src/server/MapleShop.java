...
public class MapleShop {
	...
	public void sendShop(MapleClient c) {
		...
		c.announce(MaplePacketCreator.getNPCShop(c, this.getNpcId(), this.items));
	}

	public void buy(MapleClient c, short slot, int itemId, short quantity) {
		final MapleShopItem item = this.findBySlot(slot);
		if (item != null) {
			if (item.getItemId() != itemId) {
				System.out.println("Wrong slot number in shop " + this.id);
				return;
			}
		} else {
			return;
		...
		if ((item != null) && (item.getPrice() > 0)) {
			if (c.getPlayer().getMeso() >= ((long) item.getPrice() * quantity)) {
				if (MapleInventoryManipulator.checkSpace(c, itemId, quantity,
					...
					if (!ItemConstants.isRechargable(itemId)) { // Pets can't be
						...
						c.getPlayer().gainMeso(-(item.getPrice() * quantity),
								false);
					} else {
						final short slotMax = ii
								.getSlotMax(c, item.getItemId());
						quantity = slotMax;
						MapleInventoryManipulator.addById(c, itemId, quantity);
			...
			if (c.getPlayer().getInventory(MapleInventoryType.ETC)
					.countById(4310000) >= ((long) item.getPitch() * quantity)) {
				if (MapleInventoryManipulator.checkSpace(c, itemId, quantity,
						"")) {
					if (!ItemConstants.isRechargable(itemId)) {
						MapleInventoryManipulator.addById(c, itemId, quantity);
						MapleInventoryManipulator.removeById(c,
								MapleInventoryType.ETC, 4310000,
								item.getPitch() * quantity, false, false);
					} else {
						final short slotMax = ii
								.getSlotMax(c, item.getItemId());
						quantity = slotMax;
		...
		} else if (c.getPlayer().getInventory(MapleInventoryType.CASH)
				.countById(this.token) != 0) {
			final int amount = c.getPlayer()
					.getInventory(MapleInventoryType.CASH)
					.countById(this.token);
			final int value = amount * this.tokenvalue;
	...
	public void sell(MapleClient c, MapleInventoryType type, short slot,
			short quantity) {
		...
		final MapleItemInformationProvider ii = MapleItemInformationProvider
				.getInstance();
		final Item item = c.getPlayer().getInventory(type).getItem((byte) slot);
		...
		if ((quantity <= iQuant) && (iQuant > 0)) {
			MapleInventoryManipulator.removeFromSlot(c, type, (byte) slot,
					quantity, false);
			double price;
			if (ItemConstants.isRechargable(item.getItemId())) {
				price = ii.getWholePrice(item.getItemId())
						/ (double) ii.getSlotMax(c, item.getItemId());
			} else {
				price = ii.getPrice(item.getItemId());
			}
			final int recvMesos = (int) Math
					.max(Math.ceil(price * quantity), 0);
	...
	public void recharge(MapleClient c, byte slot) {
		final MapleItemInformationProvider ii = MapleItemInformationProvider
				.getInstance();
		final Item item = c.getPlayer().getInventory(MapleInventoryType.USE)
				.getItem(slot);
		...
		if (item.getQuantity() < slotMax) {
			final int price = (int) Math.round(ii.getPrice(item.getItemId())
					* (slotMax - item.getQuantity()));
		...
		try {
			...
			while (rs.next()) {
				if (ItemConstants.isRechargable(rs.getInt("itemid"))) {
					final MapleShopItem starItem = new MapleShopItem((short) 1,
							rs.getInt("itemid"), rs.getInt("price"),
							rs.getInt("pitch"));
					...
					ret.addItem(new MapleShopItem((short) 1000, rs
							.getInt("itemid"), rs.getInt("price"), rs
							.getInt("pitch")));
				}
			}
			for (final Integer recharge : recharges) {
				ret.addItem(new MapleShopItem((short) 1000,
						recharge.intValue(), 0, 0));
			}
			rs.close();
			ps.close();
