...
public class MobAttackInfoFactory {
	...
	public static MobAttackInfo getMobAttackInfo(MapleMonster mob, int attack) {
		...
		synchronized (mobAttacks) {
			...
			if (ret == null) {
				MapleData mobData = dataSource.getData(StringUtil
						.getLeftPaddedStr(Integer.toString(mob.getId())
								+ ".img", '0', 11));
				if (mobData != null) {
					// MapleData infoData = mobData.getChildByPath("info");
					final String linkedmob = MapleDataTool.getString("link",
							mobData, "");
					if (!linkedmob.equals("")) {
						mobData = dataSource.getData(StringUtil
								.getLeftPaddedStr(linkedmob + ".img", '0', 11));
					}
					final MapleData attackData = mobData
							.getChildByPath("attack" + (attack + 1) + "/info");
					if (attackData != null) {
						final MapleData deadlyAttack = attackData
								.getChildByPath("deadlyAttack");
						final int mpBurn = MapleDataTool.getInt("mpBurn",
								attackData, 0);
						final int disease = MapleDataTool.getInt("disease",
								attackData, 0);
						final int level = MapleDataTool.getInt("level",
								attackData, 0);
						final int mpCon = MapleDataTool.getInt("conMP",
								attackData, 0);
