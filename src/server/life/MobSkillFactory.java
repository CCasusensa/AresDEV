...
public class MobSkillFactory {
	...
	private final static MapleDataProvider dataSource = MapleDataProviderFactory
	...
	public static MobSkill getMobSkill(final int skillId, final int level) {
		...
		try {
			final MobSkill ret = mobSkills.get(key);
			if (ret != null) {
				return ret;
			}
		} finally {
		...
		try {
			...
			if (ret == null) {
				final MapleData skillData = skillRoot.getChildByPath(skillId
						+ "/level/" + level);
				if (skillData != null) {
					final int mpCon = MapleDataTool.getInt(
							skillData.getChildByPath("mpCon"), 0);
					final List<Integer> toSummon = new ArrayList<Integer>();
					for (int i = 0; i > -1; i++) {
						if (skillData.getChildByPath(String.valueOf(i)) == null) {
							break;
						}
						toSummon.add(Integer.valueOf(MapleDataTool.getInt(
								skillData.getChildByPath(String.valueOf(i)), 0)));
					}
					final int effect = MapleDataTool.getInt("summonEffect",
							skillData, 0);
					...
					final long duration = MapleDataTool.getInt("time",
							skillData, 0) * 1000;
					final long cooltime = MapleDataTool.getInt("interval",
							skillData, 0) * 1000;
					final int iprop = MapleDataTool.getInt("prop", skillData,
							100);
					final float prop = iprop / 100;
					final int limit = MapleDataTool.getInt("limit", skillData,
							0);
