...
public class MapleMonster extends AbstractLoadedMapleLife {
	...
	private final EnumMap<MonsterStatus, MonsterStatusEffect> stati = new EnumMap<>(
			MonsterStatus.class);
	...
	private final List<Integer> stolenItems = new ArrayList<>();
	private int team;
	...
	public boolean isBoss() {
		return this.stats.isBoss() || this.isHT();
	}
	...
	private List<Integer> getRevives() {
		return this.stats.getRevives();
	}

	private byte getTagColor() {
		return this.stats.getTagColor();
	}

	private byte getTagBgColor() {
		return this.stats.getTagBgColor();
	}
	...
	public void damage(MapleCharacter from, int damage, boolean updateAttackTime) {
		...
		if (from.getParty() != null) {
			attacker = new PartyAttackerEntry(from.getParty().getId(), from
					.getClient().getChannelServer());
		} else {
			attacker = new SingleAttackerEntry(from, from.getClient()
					.getChannelServer());
		}
		...
		this.hp -= rDamage;
		int remhppercentage = (int) Math.ceil((this.hp * 100.0)
				/ this.getMaxHp());
		if (remhppercentage < 1) {
			remhppercentage = 1;
		}
		final long okTime = System.currentTimeMillis() - 4000;
		if (this.hasBossHPBar()) {
			from.getMap().broadcastMessage(this.makeBossHPBarPacket(),
					this.getPosition());
		} else if (!this.isBoss()) {
			for (final AttackerEntry mattacker : this.attackers) {
				for (final AttackingMapleCharacter cattacker : mattacker
						.getAttackers()) {
	...
	public void heal(int hp, int mp) {
		int hp2Heal = this.getHp() + hp;
		int mp2Heal = this.getMp() + mp;
		if (hp2Heal >= this.getMaxHp()) {
			hp2Heal = this.getMaxHp();
		}
		if (mp2Heal >= this.getMaxMp()) {
			mp2Heal = this.getMaxMp();
		}
		this.setHp(hp2Heal);
		this.setMp(mp2Heal);
		this.getMap().broadcastMessage(
				MaplePacketCreator.healMonster(this.getObjectId(), hp));
	}
	...
	public void giveExpToCharacter(MapleCharacter attacker, int exp,
			boolean highestDamage, int numExpSharers) {
		if (highestDamage) {
		...
		if (attacker.getHp() > 0) {
			int personalExp = exp;
			if (exp > 0) {
				final Integer holySymbol = attacker
						.getBuffedValue(MapleBuffStat.HOLY_SYMBOL);
				if (holySymbol != null) {
					if (numExpSharers == 1) {
				...
				if (this.stati.containsKey(MonsterStatus.SHOWDOWN)) {
					personalExp *= ((this.stati.get(MonsterStatus.SHOWDOWN)
							.getStati().get(MonsterStatus.SHOWDOWN)
	...
	public MapleCharacter killBy(MapleCharacter killer) {
		final long totalBaseExpL = (this.getExp() * killer.getClient()
				.getPlayer().getExpRate());
		final int totalBaseExp = (int) (Math.min(Integer.MAX_VALUE,
				totalBaseExpL));
		...
		for (final AttackerEntry attackEntry : this.attackers) {
			attackEntry.killedMob(
					killer.getMap(),
					(int) Math.ceil(totalBaseExp
							* ((double) attackEntry.getDamage() / this
									.getMaxHp())), attackEntry == highest);
		}
		if (this.getController() != null) { // this can/should only happen when
			...
			this.getController()
					.getClient()
					.announce(
							MaplePacketCreator.stopControllingMonster(this
									.getObjectId()));
			this.getController().stopControllingMonster(this);
		}
		final List<Integer> toSpawn = this.getRevives();
		if (toSpawn != null) {
			final MapleMap reviveMap = killer.getMap();
			if (toSpawn.contains(9300216) && (reviveMap.getId() > 925000000)
					&& (reviveMap.getId() < 926000000)) {
				reviveMap.broadcastMessage(MaplePacketCreator
						.playSound("Dojang/clear"));
				reviveMap.broadcastMessage(MaplePacketCreator
						.showEffect("dojang/end/clear"));
			}
			final Pair<Integer, String> timeMob = reviveMap.getTimeMob();
			if (timeMob != null) {
				if (toSpawn.contains(timeMob.getLeft())) {
					reviveMap.broadcastMessage(MaplePacketCreator.serverNotice(
							6, timeMob.getRight()));
				}

				if ((timeMob.getLeft() == 9300338)
						&& ((reviveMap.getId() >= 922240100) && (reviveMap
								.getId() <= 922240119))) {
					if (!reviveMap.containsNPC(9001108)) {
						final MapleNPC npc = MapleLifeFactory.getNPC(9001108);
		...
		for (final MonsterListener listener : this.listeners
				.toArray(new MonsterListener[this.listeners.size()])) {
			listener.monsterKilled(this, this.highestDamageChar);
		}
		final MapleCharacter ret = this.highestDamageChar;
	...
	public boolean isControllerHasAggro() {
		return this.fake ? false : this.controllerHasAggro;
	}
	...
	public boolean isControllerKnowsAboutAggro() {
		return this.fake ? false : this.controllerKnowsAboutAggro;
	}
	...
	public byte[] makeBossHPBarPacket() {
		return MaplePacketCreator.showBossHP(this.getId(), this.getHp(),
				this.getMaxHp(), this.getTagColor(), this.getTagBgColor());
	}

	public boolean hasBossHPBar() {
		return (this.isBoss() && (this.getTagColor() > 0)) || this.isHT();
	}
	...
	public void sendSpawnData(MapleClient c) {
		...
		if (this.stati.size() > 0) {
			for (final MonsterStatusEffect mse : this.stati.values()) {
				c.announce(MaplePacketCreator.applyMonsterStatus(
						this.getObjectId(), mse));
			}
		}
		if (this.hasBossHPBar()) {
			if ((this.getMap().countMonster(8810026) > 2)
					&& (this.getMap().getId() == 240060200)) {
				this.getMap().killAllMonsters();
				return;
			}
			c.announce(this.makeBossHPBarPacket());
		}
	}

	@Override
	public void sendDestroyData(MapleClient client) {
		client.announce(MaplePacketCreator.killMonster(this.getObjectId(),
				false));
	}
	...
	public ElementalEffectiveness getEffectiveness(Element e) {
		if ((this.stati.size() > 0)
				&& (this.stati.get(MonsterStatus.DOOM) != null)) {
	...
	public boolean applyStatus(MapleCharacter from,
			final MonsterStatusEffect status, boolean poison, long duration) {
		return this.applyStatus(from, status, poison, duration, false);
	}

	public boolean applyStatus(MapleCharacter from,
			final MonsterStatusEffect status, boolean poison, long duration,
			boolean venom) {
		switch (this.stats.getEffectiveness(status.getSkill().getElement())) {
		case IMMUNE:
		case STRONG:
		case NEUTRAL:
			return false;
		...
		default: {
			System.out.println("Unknown elemental effectiveness: "
					+ this.stats.getEffectiveness(status.getSkill()
							.getElement()));
			return false;
		}
		}
		if (status.getSkill().getId() == 2111006) { // fp compo
			final ElementalEffectiveness effectiveness = this.stats
					.getEffectiveness(Element.POISON);
		...
		} else if ((status.getSkill().getId() == 4120005)
				|| (status.getSkill().getId() == 4220005)
				|| (status.getSkill().getId() == 14110004)) {// venom
			if (this.stats.getEffectiveness(Element.POISON) == ElementalEffectiveness.WEAK) {
				return false;
			}
		}
		...
		if (this.stats.isBoss()) {
			if (!(statis.containsKey(MonsterStatus.SPEED)
					&& statis.containsKey(MonsterStatus.NINJA_AMBUSH) && statis
						.containsKey(MonsterStatus.WATK))) {
				return false;
			}
		}
		...
		final Runnable cancelTask = new Runnable() {
			...
			public void run() {
				if (MapleMonster.this.isAlive()) {
					final byte[] packet = MaplePacketCreator
							.cancelMonsterStatus(
									MapleMonster.this.getObjectId(),
									status.getStati());
					MapleMonster.this.map.broadcastMessage(packet,
							MapleMonster.this.getPosition());
					if ((MapleMonster.this.getController() != null)
							&& !MapleMonster.this.getController()
									.isMapObjectVisible(MapleMonster.this)) {
						MapleMonster.this.getController().getClient()
								.announce(packet);
					}
				}
				for (final MonsterStatus stat : status.getStati().keySet()) {
					MapleMonster.this.stati.remove(stat);
				}
				MapleMonster.this.setVenomMulti(0);
				status.cancelDamageSchedule();
			}
		};
		if (poison) {
			final int poisonLevel = from.getSkillLevel(status.getSkill());
			final int poisonDamage = Math.min(Short.MAX_VALUE,
					(int) ((this.getMaxHp() / (70.0 - poisonLevel)) + 0.999));
			status.setValue(MonsterStatus.POISON, Integer.valueOf(poisonDamage));
			status.setDamageSchedule(timerManager.register(new DamageTask(
					poisonDamage, from, status, cancelTask, 0), 1000, 1000));
		} else if (venom) {
			if ((from.getJob() == MapleJob.NIGHTLORD)
					|| (from.getJob() == MapleJob.SHADOWER)
					|| from.getJob().isA(MapleJob.NIGHTWALKER3)) {
				int poisonLevel, matk;
				final int id = from.getJob().getId();
				final int skill = (id == 412 ? 4120005 : (id == 422 ? 4220005
						: 14110004));
				poisonLevel = from.getSkillLevel(SkillFactory.getSkill(skill));
				if (poisonLevel <= 0) {
					return false;
				}
				matk = SkillFactory.getSkill(skill).getEffect(poisonLevel)
						.getMatk();
				final int luk = from.getLuk();
				final int maxDmg = (int) Math.ceil(Math.min(Short.MAX_VALUE,
						0.2 * luk * matk));
				final int minDmg = (int) Math.ceil(Math.min(Short.MAX_VALUE,
						0.1 * luk * matk));
				...
				status.setValue(MonsterStatus.POISON,
				...
				|| (status.getSkill().getId() == 14111001)) { // Shadow Web
			status.setDamageSchedule(timerManager.schedule(new DamageTask(
					(int) ((this.getMaxHp() / 50.0) + 0.999), from, status,
					cancelTask, 1), 3500));
		} else if ((status.getSkill().getId() == 4121004)
				|| (status.getSkill().getId() == 4221004)) { // Ninja Ambush
			final Skill skill = SkillFactory
					.getSkill(status.getSkill().getId());
			final byte level = from.getSkillLevel(skill);
			final int damage = (int) ((from.getStr() + from.getLuk())
					* (1.5 + (level * 0.05)) * skill.getEffect(level)
					.getDamage());
			/*
			...
			status.setDamageSchedule(timerManager.register(new DamageTask(
					damage, from, status, cancelTask, 2), 1000, 1000));
		}
		for (final MonsterStatus stat : status.getStati().keySet()) {
			this.stati.put(stat, status);
		}
		final int animationTime = status.getSkill().getAnimationTime();
		final byte[] packet = MaplePacketCreator.applyMonsterStatus(
				this.getObjectId(), status);
		this.map.broadcastMessage(packet, this.getPosition());
		if ((this.getController() != null)
				&& !this.getController().isMapObjectVisible(this)) {
			this.getController().getClient().announce(packet);
		}
	...
	public void applyMonsterBuff(final Map<MonsterStatus, Integer> stats,
			final int x, int skillId, long duration, MobSkill skill,
		...
		final Runnable cancelTask = new Runnable() {
			...
			public void run() {
				if (MapleMonster.this.isAlive()) {
					final byte[] packet = MaplePacketCreator
							.cancelMonsterStatus(
									MapleMonster.this.getObjectId(), stats);
					MapleMonster.this.map.broadcastMessage(packet,
							MapleMonster.this.getPosition());
					if ((MapleMonster.this.getController() != null)
							&& !MapleMonster.this.getController()
		...
		final MonsterStatusEffect effect = new MonsterStatusEffect(stats, null,
				skill, true);
		final byte[] packet = MaplePacketCreator.applyMonsterStatus(
				this.getObjectId(), effect);
		this.map.broadcastMessage(packet, this.getPosition());
		if ((this.getController() != null)
				&& !this.getController().isMapObjectVisible(this)) {
			this.getController().getClient().announce(packet);
	...
	public boolean canUseSkill(MobSkill toUse) {
		...
		for (final Pair<Integer, Integer> skill : this.usedSkills) {
		...
		if (toUse.getLimit() > 0) {
			if (this.skillsUsed.containsKey(new Pair<>(toUse.getSkillId(),
					toUse.getSkillLevel()))) {
				final int times = this.skillsUsed.get(new Pair<>(toUse
						.getSkillId(), toUse.getSkillLevel()));
		...
		if (toUse.getSkillId() == 200) {
			final Collection<MapleMapObject> mmo = this.getMap()
					.getMapObjects();
			int i = 0;
			for (final MapleMapObject mo : mmo) {
	...
	public void usedSkill(final int skillId, final int level, long cooltime) {
		...
		if (this.skillsUsed.containsKey(new Pair<>(skillId, level))) {
			final int times = this.skillsUsed.get(new Pair<>(skillId, level)) + 1;
	...
	public void clearSkill(int skillId, int level) {
		...
		for (final Pair<Integer, Integer> skill : this.usedSkills) {
			if ((skill.getLeft() == skillId) && (skill.getRight() == level)) {
				index = this.usedSkills.indexOf(skill);
				break;
			}
		}
		if (index != -1) {
			this.usedSkills.remove(index);
		}
	}
	...
	private final class DamageTask implements Runnable {
		...
		@Override
		public void run() {
			int damage = this.dealDamage;
			if (damage >= MapleMonster.this.hp) {
				damage = MapleMonster.this.hp - 1;
				if ((this.type == 1) || (this.type == 2)) {
					this.map.broadcastMessage(
							MaplePacketCreator.damageMonster(
									MapleMonster.this.getObjectId(), damage),
							MapleMonster.this.getPosition());
					this.cancelTask.run();
					this.status.getCancelTask().cancel(false);
				}
			}
			if ((MapleMonster.this.hp > 1) && (damage > 0)) {
	...
	private class SingleAttackerEntry implements AttackerEntry {
		...
		public void addDamage(MapleCharacter from, int damage,
			...
			if (this.chrid == from.getId()) {
				...
				throw new IllegalArgumentException(
						"Not the attacker of this entry");
			}
			if (updateAttackTime) {
		...
		public List<AttackingMapleCharacter> getAttackers() {
			final MapleCharacter chr = this.cserv.getPlayerStorage()
					.getCharacterById(this.chrid);
			if (chr != null) {
				return Collections.singletonList(new AttackingMapleCharacter(
						chr, this.lastAttackTime));
			} else {
		...
		public void killedMob(MapleMap map, int baseExp, boolean mostDamage) {
			...
			if (chr != null) {
				MapleMonster.this.giveExpToCharacter(chr, baseExp, mostDamage,
						1);
			}
		}
	...
	private class PartyAttackerEntry implements AttackerEntry {
		...
		public PartyAttackerEntry(int partyid, Channel cserv) {
			...
			this.cserv = cserv;
			this.attackers = new HashMap<>(6);
		}

		@Override
		public List<AttackingMapleCharacter> getAttackers() {
			final List<AttackingMapleCharacter> ret = new ArrayList<>(
					this.attackers.size());
			for (final Entry<Integer, OnePartyAttacker> entry : this.attackers
					.entrySet()) {
				final MapleCharacter chr = this.cserv.getPlayerStorage()
		...
		private Map<MapleCharacter, OnePartyAttacker> resolveAttackers() {
			final Map<MapleCharacter, OnePartyAttacker> ret = new HashMap<>(
					this.attackers.size());
			for (final Entry<Integer, OnePartyAttacker> aentry : this.attackers
					.entrySet()) {
				final MapleCharacter chr = this.cserv.getPlayerStorage()
		...
		public void addDamage(MapleCharacter from, int damage,
				boolean updateAttackTime) {
			final OnePartyAttacker oldPartyAttacker = this.attackers.get(from
					.getId());
			if (oldPartyAttacker != null) {
				oldPartyAttacker.damage += damage;
				oldPartyAttacker.lastKnownParty = from.getParty();
				if (updateAttackTime) {
					oldPartyAttacker.lastAttackTime = System
							.currentTimeMillis();
				}
			} else {
				...
				// future but not now
				final OnePartyAttacker onePartyAttacker = new OnePartyAttacker(
						from.getParty(), damage);
		...
		@Override
		public void killedMob(MapleMap map, int baseExp, boolean mostDamage) {
			final Map<MapleCharacter, OnePartyAttacker> attackers_ = this
					.resolveAttackers();
			MapleCharacter highest = null;
			int highestDamage = 0;
			final Map<MapleCharacter, Integer> expMap = new ArrayMap<>(6);
			for (final Entry<MapleCharacter, OnePartyAttacker> attacker : attackers_
					.entrySet()) {
				final MapleParty party = attacker.getValue().lastKnownParty;
				double averagePartyLevel = 0;
				final List<MapleCharacter> expApplicable = new ArrayList<>();
				for (final MaplePartyCharacter partychar : party.getMembers()) {
					if (((attacker.getKey().getLevel() - partychar.getLevel()) <= 5)
							|| ((MapleMonster.this.getLevel() - partychar
									.getLevel()) <= 5)) {
						final MapleCharacter pchr = this.cserv
				...
				final double innerBaseExp = baseExp
						* ((double) iDamage / this.totDamage);
				final double expFraction = (innerBaseExp * expBonus)
						/ (expApplicable.size() + 1);
				for (final MapleCharacter expReceiver : expApplicable) {
					...
					final double expWeight = (expReceiver == attacker.getKey() ? 2.0
							: 1.0);
					double levelMod = expReceiver.getLevel()
							/ averagePartyLevel;
					if ((levelMod > 1.0)
							|| this.attackers.containsKey(expReceiver.getId())) {
						levelMod = 1.0;
					}
					iexp += (int) Math
							.round(expFraction * expWeight * levelMod);
					expMap.put(expReceiver, Integer.valueOf(iexp));
				}
			}
			for (final Entry<MapleCharacter, Integer> expReceiver : expMap
					.entrySet()) {
				MapleMonster.this.giveExpToCharacter(expReceiver.getKey(),
	...
	public void addStolen(int itemId) {
		this.stolenItems.add(itemId);
	}

	public void setTempEffectiveness(Element e, ElementalEffectiveness ee,
			long milli) {
		final Element fE = e;
		final ElementalEffectiveness fEE = this.stats.getEffectiveness(e);
		if (!this.stats.getEffectiveness(e).equals(ElementalEffectiveness.WEAK)) {
			this.stats.setEffectiveness(e, ee);
			TimerManager.getInstance().schedule(new Runnable() {

				@Override
				public void run() {
					MapleMonster.this.stats.removeEffectiveness(fE);
					MapleMonster.this.stats.setEffectiveness(fE, fEE);
				}
			}, milli);
		}
	}
	...
	public BanishInfo getBanish() {
		return this.stats.getBanishInfo();
	}
	...
	public int getDropPeriodTime() {
		return this.stats.getDropPeriod();
	}

	public int getPADamage() {
		return this.stats.getPADamage();
	}
