...
public class MakerItemFactory {
		...
		if (createCache.get(toCreate) != null) {
			...
			try {
				...
				PreparedStatement ps = con
						.prepareStatement("SELECT req_level, req_maker_level, req_meso, quantity FROM makercreatedata WHERE itemid = ?");
				...
				ps.close();
				rs.close();
				final MakerItemCreateEntry ret = new MakerItemCreateEntry(cost,
						reqLevel, reqMakerLevel, toGive);
	...
	public static class MakerItemCreateEntry {
		...
		private final List<Pair<Integer, Integer>> reqItems = new ArrayList<Pair<Integer, Integer>>(); // itemId
																										// /
																										// amount
		private final int toGive;

		private MakerItemCreateEntry(int cost, int reqLevel, int reqMakerLevel,
				int toGive) {
			this.cost = cost;
