...
public class MapleInventoryManipulator {
	...
	public static boolean addById(MapleClient c, int itemId, short quantity,
			String owner, int petid) {
		return addById(c, itemId, quantity, owner, petid, -1);
	}

	public static boolean addById(MapleClient c, int itemId, short quantity,
			String owner, int petid, long expiration) {
		return addById(c, itemId, quantity, owner, petid, (byte) 0, expiration);
	}

	public static boolean addById(MapleClient c, int itemId, short quantity,
			String owner, int petid, byte flag, long expiration) {
		...
		if (!type.equals(MapleInventoryType.EQUIP)) {
			...
			final List<Item> existing = c.getPlayer().getInventory(type)
					.listById(itemId);
			if (!ItemConstants.isRechargable(itemId)) {
					...
					while (quantity > 0) {
						if (i.hasNext()) {
							...
							if ((oldQ < slotMax)
								...
								c.announce(MaplePacketCreator.modifyInventory(
										true,
										Collections
												.singletonList(new ModifyInventory(
														1, eItem))));
							}
						} else {
							break;
				...
				while ((quantity > 0) || ItemConstants.isRechargable(itemId)) {
					...
					if (newQ != 0) {
						quantity -= newQ;
						final Item nItem = new Item(itemId, (byte) 0, newQ,
								petid);
						nItem.setFlag(flag);
						nItem.setExpiration(expiration);
						final byte newSlot = c.getPlayer().getInventory(type)
								.addItem(nItem);
						if (newSlot == -1) {
						...
						c.announce(MaplePacketCreator.modifyInventory(true,
								Collections.singletonList(new ModifyInventory(
										0, nItem))));
						if ((ItemConstants.isRechargable(itemId))
								&& (quantity == 0)) {
							break;
				...
				final Item nItem = new Item(itemId, (byte) 0, quantity, petid);
				nItem.setFlag(flag);
				nItem.setExpiration(expiration);
				final byte newSlot = c.getPlayer().getInventory(type)
						.addItem(nItem);
				...
				c.announce(MaplePacketCreator.modifyInventory(true, Collections
						.singletonList(new ModifyInventory(0, nItem))));
			}
		} else if (quantity == 1) {
	...
	public static boolean addFromDrop(MapleClient c, Item item, boolean show) {
		...
		if (ii.isPickupRestricted(item.getItemId())
				&& (c.getPlayer().getItemQuantity(item.getItemId(), true) > 0)) {
			c.announce(MaplePacketCreator.getInventoryFull());
		...
		if (!type.equals(MapleInventoryType.EQUIP)) {
			...
			if (!ItemConstants.isRechargable(item.getItemId())) {
				...
				while ((quantity > 0)
					...
					final short newQ = (short) Math.min(quantity, slotMax);
					quantity -= newQ;
					final Item nItem = new Item(item.getItemId(), (byte) 0,
							newQ);
					...
					c.announce(MaplePacketCreator.modifyInventory(true,
							Collections.singletonList(new ModifyInventory(0,
									nItem))));
					if ((ItemConstants.isRechargable(item.getItemId()))
							&& (quantity == 0)) {
						break;
		...
		if (show) {
			c.announce(MaplePacketCreator.getShowItemGain(item.getItemId(),
					item.getQuantity()));
		}
		return true;
	}

	public static boolean checkSpace(MapleClient c, int itemid, int quantity,
			String owner) {
		final MapleItemInformationProvider ii = MapleItemInformationProvider
				.getInstance();
		final MapleInventoryType type = ii.getInventoryType(itemid);
		if (!type.equals(MapleInventoryType.EQUIP)) {
			...
			if (slotMax > 0) {
				numSlotsNeeded = (int) (Math
						.ceil(((double) quantity) / slotMax));
	...
	public static void removeFromSlot(MapleClient c, MapleInventoryType type,
			byte slot, short quantity, boolean fromDrop) {
		removeFromSlot(c, type, slot, quantity, fromDrop, false);
	}

	public static void removeFromSlot(MapleClient c, MapleInventoryType type,
			byte slot, short quantity, boolean fromDrop, boolean consume) {
		final Item item = c.getPlayer().getInventory(type).getItem(slot);
		final boolean allowZero = consume
				&& ItemConstants.isRechargable(item.getItemId());
		c.getPlayer().getInventory(type).removeItem(slot, quantity, allowZero);
		if ((item.getQuantity() == 0) && !allowZero) {
			c.announce(MaplePacketCreator.modifyInventory(fromDrop,
					Collections.singletonList(new ModifyInventory(3, item))));
		} else {
			c.announce(MaplePacketCreator.modifyInventory(fromDrop,
					Collections.singletonList(new ModifyInventory(1, item))));
		}
	}

	public static void removeById(MapleClient c, MapleInventoryType type,
			int itemId, int quantity, boolean fromDrop, boolean consume) {
		final List<Item> items = c.getPlayer().getInventory(type)
				.listById(itemId);
		int remremove = quantity;
		for (final Item item : items) {
			if (remremove <= item.getQuantity()) {
				removeFromSlot(c, type, item.getPosition(), (short) remremove,
						fromDrop, consume);
				remremove = 0;
				break;
			} else {
				remremove -= item.getQuantity();
				removeFromSlot(c, type, item.getPosition(), item.getQuantity(),
						fromDrop, consume);
			}
		}
		if (remremove > 0) {
			throw new RuntimeException("[h4x] Not enough items available ("
					+ itemId + ", " + (quantity - remremove) + "/" + quantity
					+ ")");
		}
	}

	public static void move(MapleClient c, MapleInventoryType type, byte src,
			byte dst) {
		if ((src < 0) || (dst < 0)) {
			return;
		}
		...
		final Item initialTarget = c.getPlayer().getInventory(type)
				.getItem(dst);
		if (source == null) {
			return;
		}
	...
	public static void equip(MapleClient c, byte src, byte dst) {
		Equip source = (Equip) c.getPlayer()
				.getInventory(MapleInventoryType.EQUIP).getItem(src);
		if ((source == null)
				|| !MapleItemInformationProvider.getInstance()
						.canWearEquipment(c.getPlayer(), source)) {
			c.announce(MaplePacketCreator.enableActions());
			return;
		} else if (((((source.getItemId() >= 1902000) && (source.getItemId() <= 1902002)) || (source
				.getItemId() == 1912000)) && c.getPlayer().isCygnus())
				|| ((((source.getItemId() >= 1902005) && (source.getItemId() <= 1902007)) || (source
						.getItemId() == 1912005)) && !c.getPlayer().isCygnus())) {// Adventurer
																					// taming
																					// equipment
			return;
		}
		boolean itemChanged = false;
		if (MapleItemInformationProvider.getInstance().isUntradeableOnEquip(
				source.getItemId())) {
		...
		if (dst == -6) { // unequip the overall
			final Item top = c.getPlayer()
					.getInventory(MapleInventoryType.EQUIPPED)
					.getItem((byte) -5);
			if ((top != null) && isOverall(top.getItemId())) {
				if (c.getPlayer().getInventory(MapleInventoryType.EQUIP)
						.isFull()) {
					c.announce(MaplePacketCreator.getInventoryFull());
					c.announce(MaplePacketCreator.getShowInventoryFull());
					return;
			...
			final Item weapon = c.getPlayer()
					.getInventory(MapleInventoryType.EQUIPPED)
					.getItem((byte) -11);
			if ((weapon != null)
					&& MapleItemInformationProvider.getInstance().isTwoHanded(
							weapon.getItemId())) {
				if (c.getPlayer().getInventory(MapleInventoryType.EQUIP)
						.isFull()) {
					c.announce(MaplePacketCreator.getInventoryFull());
		...
		source = (Equip) c.getPlayer().getInventory(MapleInventoryType.EQUIP)
				.getItem(src);
		final Equip target = (Equip) c.getPlayer()
		...
		if (itemChanged) {
			mods.add(new ModifyInventory(3, source));
			mods.add(new ModifyInventory(0, source.copy()));// to prevent
															// crashes
		}
	...
	public static void unequip(MapleClient c, byte src, byte dst) {
		final Equip source = (Equip) c.getPlayer()
				.getInventory(MapleInventoryType.EQUIPPED).getItem(src);
		final Equip target = (Equip) c.getPlayer()
		...
		c.announce(MaplePacketCreator.modifyInventory(true,
				Collections.singletonList(new ModifyInventory(2, source, src))));
		c.getPlayer().equipChanged();
	}

	public static void drop(MapleClient c, MapleInventoryType type, byte src,
			short quantity) {
		final MapleItemInformationProvider ii = MapleItemInformationProvider
				.getInstance();
		if (src < 0) {
		...
		if ((c.getPlayer().getItemEffect() == itemId)
		...
		if ((c.getPlayer().getItemQuantity(itemId, true) < quantity)
				|| (quantity < 0) || (source == null)
				|| ((quantity == 0) && !ItemConstants.isRechargable(itemId))) {
			return;
		}
		final Point dropPos = new Point(c.getPlayer().getPosition());
		if ((quantity < source.getQuantity())
				&& !ItemConstants.isRechargable(itemId)) {
			final Item target = source.copy();
			target.setQuantity(quantity);
			source.setQuantity((short) (source.getQuantity() - quantity));
			c.announce(MaplePacketCreator.modifyInventory(true,
					Collections.singletonList(new ModifyInventory(1, source))));
			final boolean weddingRing = (source.getItemId() == 1112803)
			...
			if (weddingRing) {
				c.getPlayer()
						.getMap()
						.disappearingItemDrop(c.getPlayer(), c.getPlayer(),
								target, dropPos);
			} else if (c.getPlayer().getMap().getEverlast()) {
				if (ii.isDropRestricted(target.getItemId())
						|| MapleItemInformationProvider.getInstance().isCash(
								target.getItemId())) {
					c.getPlayer()
							.getMap()
							.disappearingItemDrop(c.getPlayer(), c.getPlayer(),
					...
					c.getPlayer()
							.getMap()
							.spawnItemDrop(c.getPlayer(), c.getPlayer(),
									target, dropPos, true, false);
				}
				...
				c.getPlayer()
						.getMap()
						.spawnItemDrop(c.getPlayer(), c.getPlayer(), target,
								dropPos, true, true);
			}
		} else {
			c.getPlayer().getInventory(type).removeSlot(src);
			c.announce(MaplePacketCreator.modifyInventory(true,
					Collections.singletonList(new ModifyInventory(3, source))));
			if (src < 0) {
				c.getPlayer().equipChanged();
			}
			if (c.getPlayer().getMap().getEverlast()) {
				...
				c.getPlayer()
						.getMap()
						.spawnItemDrop(c.getPlayer(), c.getPlayer(), source,
								dropPos, true, true);
			}
		}
	}
