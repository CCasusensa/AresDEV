...
public class SkillFactory {
	...
	public static void loadAllSkills() {
		...
		for (final MapleDataFileEntry topDir : root.getFiles()) { // Loop thru
																	// jobs
			if (topDir.getName().length() <= 8) {
				for (final MapleData data : datasource
						.getData(topDir.getName())) { // Loop
					// thru
					// each
					// jobs
					if (data.getName().equals("skill")) {
						for (final MapleData data2 : data) { // Loop thru each
																// jobs
							if (data2 != null) {
	...
	public static Skill loadFromData(int id, MapleData data) {
		...
		for (final MapleData level : data.getChildByPath("level")) {
			ret.effects.add(MapleStatEffect.loadSkillEffectFromData(level, id,
					isBuff));
		}
		ret.animationTime = 0;
		if (effect != null) {
			for (final MapleData effectEntry : effect) {
				ret.animationTime += MapleDataTool.getIntConvert("delay",
						effectEntry, 0);
			}
		}
		return ret;
	...
	public static String getSkillName(int skillid) {
		final MapleData data = MapleDataProviderFactory.getDataProvider(
				...
				.getData("Skill.img");
		final StringBuilder skill = new StringBuilder();
		...
		if (data.getChildByPath(skill.toString()) != null) {
			for (final MapleData skilldata : data.getChildByPath(
					skill.toString()).getChildren()) {
