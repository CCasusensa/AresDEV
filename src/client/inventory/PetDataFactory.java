...
public class PetDataFactory {
	...
	public static PetCommand getPetCommand(int petId, int skillId) {
		...
		synchronized (petCommands) {
			...
			if (ret == null) {
				final MapleData skillData = dataRoot.getData("Pet/" + petId
						+ ".img");
				int prob = 0;
				int inc = 0;
				if (skillData != null) {
					prob = MapleDataTool.getInt(
							"interact/" + skillId + "/prob", skillData, 0);
					inc = MapleDataTool.getInt("interact/" + skillId + "/inc",
							skillData, 0);
				}
	...
	public static int getHunger(int petId) {
		...
		synchronized (petHunger) {
			...
			if (ret == null) {
				ret = Integer.valueOf(MapleDataTool.getInt(
						dataRoot.getData("Pet/" + petId + ".img")
								.getChildByPath("info/hungry"), 1));
			}
			return ret;
		}
	}
