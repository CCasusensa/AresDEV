...
public class LoginHandler implements GMPacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea,
		...
		try {
			...
			rs = ps.executeQuery();
			if (rs.next()) {
				final String pw = rs.getString("password");
				if (password.equals(pw)
						|| MapleClient.checkHash(pw, "SHA-1", password)) {
					/*
					...
					session.write(GMPacketCreator.sendLoginResponse((byte) 3,
							login));
					GMServer.broadcastOutGame(
							GMPacketCreator.chat(login + " has logged in."),
							login);
					GMServer.broadcastOutGame(GMPacketCreator.addUser(login),
							login);
					session.write(GMPacketCreator.sendUserList(GMServer
							.getUserList(login)));
					return;
				}
			}
