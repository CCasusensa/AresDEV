...
public class GMServer {
	...
	public static void startGMServer() {
		...
		acceptor.getFilterChain().addLast("codec",
				new ProtocolCodecFilter(new GMCodecFactory()));
		...
		for (final World w : Server.getInstance().getWorlds()) {// For when
			for (final Channel c : w.getChannels()) {
				for (final MapleCharacter chr : c.getPlayerStorage()
						.getAllCharacters()) {
					if (chr.isGM()) {
	...
	public static void broadcastOutGame(byte[] packet, String exclude) {
		for (final IoSession ss : outGame.values()) {
	...
	public static void broadcastInGame(byte[] packet) {
		for (final IoSession ss : inGame.values()) {
			ss.write(packet);
		}
	}
	...
	public static void addInGame(String name, IoSession session) {
		if (!inGame.containsKey(name)) {
			broadcastOutGame(GMPacketCreator.chat(name + " has logged in."),
					null);
	...
	public static void removeInGame(String name) {
		if (inGame.remove(name) != null) {
			...
			broadcastOutGame(GMPacketCreator.chat(name + " has logged out."),
					null);
		}
	}

	public static void removeOutGame(String name) {
		final IoSession ss = outGame.remove(name);
		if (ss != null) {
			if (!ss.isClosing()) {
				broadcastOutGame(GMPacketCreator.removeUser(name), null);
				broadcastOutGame(
						GMPacketCreator.chat(name + " has logged out."), null);
			}
		}
	}
	...
	public static void closeAllSessions() {
		try {// I CAN AND IT'S FREE BITCHES
			final Collection<IoSession> sss = Collections
					.synchronizedCollection(outGame.values());
			synchronized (sss) {
	...
	public static List<String> getUserList(String exclude) {
		final List<String> returnList = new ArrayList<>(outGame.keySet());
