...
public final class ServerlistRequestHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		for (final World world : server.getWorlds()) {
			c.announce(MaplePacketCreator.getServerList(world.getId(),
					ServerConstants.WORLD_NAMES[world.getId()],
					world.getFlag(), world.getEventMessage(),
					world.getChannels()));
		}
		c.announce(MaplePacketCreator.getEndOfServerList());
		c.announce(MaplePacketCreator.selectWorld(0));// too lazy to make a
														// check lol
		c.announce(MaplePacketCreator.sendRecommended(server
