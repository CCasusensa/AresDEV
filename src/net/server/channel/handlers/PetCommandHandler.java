...
public final class PetCommandHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		final byte petIndex = chr.getPetIndex(petId);
		MaplePet pet;
		if (petIndex == -1) {
			return;
		} else {
		...
		final PetCommand petCommand = PetDataFactory.getPetCommand(
				pet.getItemId(), command);
		if (petCommand == null) {
			return;
		}
		...
		if (Randomizer.nextInt(101) <= petCommand.getProbability()) {
			...
			if (pet.getCloseness() < 30000) {
				...
				final Item petz = chr.getInventory(MapleInventoryType.CASH)
		...
		chr.getMap().broadcastMessage(
				c.getPlayer(),
				MaplePacketCreator.commandResponse(chr.getId(), petIndex,
