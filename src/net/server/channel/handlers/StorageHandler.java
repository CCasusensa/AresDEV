...
public final class StorageHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if (mode == 4) { // take out
			...
			if ((quantity < 1)
					|| (chr.getItemQuantity(itemId, false) < quantity)) {
				return;
			}
			if (storage.isFull()) {
			...
			final short meso = (short) (chr.getMap().getId() == 910000000 ? -500
					: -100);
			if (chr.getMeso() < meso) {
				c.announce(MaplePacketCreator.getStorageError((byte) 0x0B));
			} else {
				final MapleInventoryType type = ii.getInventoryType(itemId);
				final Item item = chr.getInventory(type).getItem(slot).copy();
				if ((item.getItemId() == itemId)
						&& ((item.getQuantity() >= quantity) || ItemConstants
								.isRechargable(itemId))) {
