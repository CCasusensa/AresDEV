...
public final class DistributeSPHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if (((skillid % 10000000) > 999) && ((skillid % 10000000) < 1003)) {
			int total = 0;
			for (int i = 0; i < 3; i++) {
				total += player.getSkillLevel(SkillFactory.getSkill((player
						.getJobType() * 10000000) + 1000 + i));
			}
		...
		if (((remainingSp > 0) && ((curLevel + 1) <= (skill.isFourthJob() ? player
			...
			player.changeSkillLevel(skill, (byte) (curLevel + 1),
					player.getMasterLevel(skill),
					player.getSkillExpiration(skill));
		}
	}
}
