...
public final class PlayerLoggedinHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		MapleCharacter player = c.getWorldServer().getPlayerStorage()
		...
		if ((state == MapleClient.LOGIN_SERVER_TRANSITION)
			...
			for (final String charName : c.loadCharacterNames(c.getWorld())) {
				for (final Channel ch : c.getWorldServer().getChannels()) {
		...
		final List<PlayerBuffValueHolder> buffs = server.getPlayerBuffStorage()
				.getBuffsFromStorage(cid);
		if (buffs != null) {
		...
		final World world = server.getWorld(c.getWorld());
		world.getPlayerStorage().addPlayer(player);

		final int buddyIds[] = player.getBuddylist().getBuddyIds();
		world.loggedOn(player.getName(), player.getId(), c.getChannel(),
				buddyIds);
		for (final CharacterIdChannelPair onlineBuddy : server.getWorld(
				c.getWorld()).multiBuddyFind(player.getId(), buddyIds)) {
			final BuddyListEntry ble = player.getBuddylist().get(
		...
		if (player.getGuildId() > 0) {
			final MapleGuild playerGuild = server.getGuild(player.getGuildId(),
					player.getMGC());
			if (playerGuild == null) {
				...
				server.setGuildMemberOnline(player.getMGC(), true,
						c.getChannel());
				...
				if (allianceId > 0) {
					...
					if (newAlliance != null) {
						c.announce(MaplePacketCreator
								.getAllianceInfo(newAlliance));
						c.announce(MaplePacketCreator.getGuildAlliances(
								newAlliance, c));
						server.allianceMessage(allianceId, MaplePacketCreator
								.allianceMemberOnline(player, true), player
								.getId(), -1);
					}
				}
			}
		...
		if (player.getParty() != null) {
			final MaplePartyCharacter pchar = player.getMPC();
			pchar.setChannel(c.getChannel());
			pchar.setMapId(player.getMapId());
			pchar.setOnline(true);
			world.updateParty(player.getParty().getId(),
					PartyOperation.LOG_ONOFF, pchar);
		}
