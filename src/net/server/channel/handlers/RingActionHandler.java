	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		case 0: // Send
			...
			final MapleCharacter partner = c.getChannelServer()
					.getPlayerStorage().getCharacterByName(partnerName);
		...
		case 1: // Cancel send
			...
			final String proposerName = slea.readMapleAsciiString();
			if (accepted) {
				c.announce(MaplePacketCreator
						.sendEngagementRequest(proposerName));
			}
			break;
		case 2:
			slea.readByte();
		case 3: // Drop Ring
			 ...
			 * con.prepareStatement("DELETE FROM engagements WHERE husbandid = ?"
			 * ); ps.setInt(1, pid); ps.executeUpdate(); ps.close(); ps =
