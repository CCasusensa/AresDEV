...
public final class SkillBookHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if ((toUse != null) && (toUse.getQuantity() == 1)) {
			...
			final Map<String, Integer> skilldata = MapleItemInformationProvider
			...
			if (skilldata.get("skillid") == 0) {
				canuse = false;
			} else if ((player.getMasterLevel(SkillFactory.getSkill(skilldata
					.get("skillid"))) >= skilldata.get("reqSkillLevel"))
					|| (skilldata.get("reqSkillLevel") == 0)) {
				canuse = true;
				if ((Randomizer.nextInt(101) < skilldata.get("success"))
						&& (skilldata.get("success") != 0)) {
					success = true;
					final Skill skill2 = SkillFactory.getSkill(skilldata
							.get("skillid"));
					player.changeSkillLevel(
							skill2,
							player.getSkillLevel(skill2),
							Math.max(skilldata.get("masterLevel"),
									player.getMasterLevel(skill2)), -1);
				} else {
					success = false;
			...
			player.getClient().announce(
					MaplePacketCreator.skillBookSuccess(player, skill,
