...
public final class GuildOperationHandler extends AbstractMaplePacketHandler {
	...
	private class Invited {
		...
		public Invited(String n, int id) {
			...
			this.gid = id;
			this.expiration = System.currentTimeMillis() + (60 * 60 * 1000);
		}
	...
	private final java.util.List<Invited> invited = new java.util.LinkedList<Invited>();
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if (System.currentTimeMillis() >= this.nextPruneTime) {
			final Iterator<Invited> itr = this.invited.iterator();
			Invited inv;
			while (itr.hasNext()) {
				inv = itr.next();
			...
			this.nextPruneTime = System.currentTimeMillis() + (20 * 60 * 1000);
		}
		...
		case 0x02:
			...
			if (!this.isGuildNameAcceptable(guildName)) {
				c.getPlayer().dropMessage(1,
		...
		case 0x07:
			...
			if ((cid != mc.getId()) || !name.equals(mc.getName())
					|| (mc.getGuildId() <= 0)) {
				System.out
						.println("[hax] " + mc.getName()
								+ " tried to quit guild under the name \""
								+ name + "\" and current guild id of "
								+ mc.getGuildId() + ".");
				return;
			}
		...
		case 0x08:
			...
			if ((mc.getGuildRank() > 2) || (mc.getGuildId() <= 0)) {
				System.out.println("[hax] " + mc.getName()
						+ " is trying to expel without rank 1 or 2.");
				return;
			}
		...
		case 0x0f:
			...
			Server.getInstance().setGuildEmblem(mc.getGuildId(), bg, bgcolor,
					logo, logocolor);
