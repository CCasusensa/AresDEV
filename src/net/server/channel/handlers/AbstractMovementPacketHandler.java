...
public abstract class AbstractMovementPacketHandler extends
	...
	protected List<LifeMovementFragment> parseMovement(LittleEndianAccessor lea) {
		...
		for (byte i = 0; i < numCommands; i++) {
			...
			case 14: { // Before Jump Down - fixes item/mobs dissappears
				 ...
				 * ywobble = lea.readShort(); byte newstate = lea.readByte();
				 * TeleportMovement tm = new TeleportMovement(command, new
				 * Point(xpos, ypos), newstate); tm.setPixelsPerSecond(new
				 * Point(xwobble, ywobble)); res.add(tm); break; }
				 */
			}
			...
			case 21: {// Causes aran to do weird stuff when attacking o.o
				/*
				 * byte newstate = lea.readByte(); short unk = lea.readShort();
				 * AranMovement am = new AranMovement(command, null, unk,
				 * newstate); res.add(am);
				 */
				lea.skip(3);
				break;
	...
	protected void updatePosition(List<LifeMovementFragment> movement,
