		...
		if (c.getPlayer().getInventory(MapleInventoryType.USE).findById(itemid) != null) {
			if ((c.getPlayer().getMount() != null)
				...
				c.getPlayer()
						.getMount()
						.setTiredness(
								Math.max(c.getPlayer().getMount()
										.getTiredness() - 30, 0));
				c.getPlayer()
						.getMount()
						.setExp((2 * c.getPlayer().getMount().getLevel()) + 6
								+ c.getPlayer().getMount().getExp());
				final int level = c.getPlayer().getMount().getLevel();
				final boolean levelup = (c.getPlayer().getMount().getExp() >= ExpTable
						.getMountExpNeededForLevel(level)) && (level < 31);
				if (levelup) {
				...
				c.getPlayer()
						.getMap()
						.broadcastMessage(
								MaplePacketCreator.updateMount(c.getPlayer()
										.getId(), c.getPlayer().getMount(),
										levelup));
