...
public class FredrickHandler extends AbstractMaplePacketHandler {
			...
			try {
				...
				if (deleteItems(chr)) {
					for (int i = 0; i < items.size(); i++) {
						MapleInventoryManipulator.addFromDrop(c, items.get(i)
								.getLeft(), false);
					}
					c.announce(MaplePacketCreator.fredrickMessage((byte) 0x1E));
				} else {
					chr.addMessage("An unknown error has occured.");
				}
				break;
	...
	private static boolean check(MapleCharacter chr,
		...
		for (final Pair<Item, MapleInventoryType> item : items) {
			if (!MapleInventoryManipulator.checkSpace(chr.getClient(), item
					.getLeft().getItemId(), item.getLeft().getQuantity(), item
					.getLeft().getOwner())) {
				return false;
			}
		}
