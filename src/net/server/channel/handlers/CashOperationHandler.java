...
public final class CashOperationHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if ((action == 0x03) || (action == 0x1E)) {
			...
			final CashItem cItem = CashItemFactory.getItem(snCS);
			if ((cItem == null) || !cItem.isOnSale()
					|| (cs.getCash(useNX) < cItem.getPrice())) {
				return;
			}
			if (action == 0x03) { // Item
				final Item item = cItem.toItem();
				cs.addToInventory(item);
				c.announce(MaplePacketCreator.showBoughtCashItem(item,
						c.getAccID()));
			} else { // Package
				final List<Item> cashPackage = CashItemFactory.getPackage(cItem
						.getItemId());
				for (final Item item : cashPackage) {
					cs.addToInventory(item);
				}
				c.announce(MaplePacketCreator.showBoughtCashPackage(
						cashPackage, c.getAccID()));
			}
			cs.gainCash(useNX, -cItem.getPrice());
			c.announce(MaplePacketCreator.showCash(chr));
		} else if (action == 0x04) {// TODO check for gender
			final int birthday = slea.readInt();
			final CashItem cItem = CashItemFactory.getItem(slea.readInt());
			final Map<String, String> recipient = MapleCharacter
					.getCharacterFromDatabase(slea.readMapleAsciiString());
			final String message = slea.readMapleAsciiString();
			if (!canBuy(cItem, cs.getCash(4)) || (message.length() < 1)
					|| (message.length() > 73)) {
				return;
			}
			if (!checkBirthday(c, birthday)) {
				...
				return;
			} else if (recipient.get("accountid").equals(
					String.valueOf(c.getAccID()))) {
				c.announce(MaplePacketCreator.showCashShopMessage((byte) 0xA8));
				return;
			}
			cs.gift(Integer.parseInt(recipient.get("id")), chr.getName(),
					message, cItem.getSN());
			c.announce(MaplePacketCreator.showGiftSucceed(
					recipient.get("name"), cItem));
			cs.gainCash(4, -cItem.getPrice());
			c.announce(MaplePacketCreator.showCash(chr));
			try {
				chr.sendNote(recipient.get("name"), chr.getName()
						+ " has sent you a gift! Go check out the Cash Shop.",
						(byte) 0); // fame or not
			} catch (final SQLException ex) {
			}
			final MapleCharacter receiver = c.getChannelServer()
					.getPlayerStorage()
			...
			for (byte i = 0; i < 10; i++) {
				...
				final CashItem cItem = CashItemFactory.getItem(sn);
			...
			if (mode == 0) {
				...
				if (chr.gainSlots(type, 4, false)) {
					c.announce(MaplePacketCreator.showBoughtInventorySlots(
							type, chr.getSlots(type)));
			...
			if (mode == 0) {
				...
				if (chr.getStorage().gainSlots(4)) {
					c.announce(MaplePacketCreator.showBoughtStorageSlots(chr
							.getStorage().getSlots()));
					cs.gainCash(cash, -4000);
					c.announce(MaplePacketCreator.showCash(chr));
				}
			...
			if (c.gainCharacterSlot()) {
				c.announce(MaplePacketCreator.showBoughtCharacterSlot(c
						.getCharacterSlots()));
				cs.gainCash(cash, -cItem.getPrice());
				c.announce(MaplePacketCreator.showCash(chr));
			}
		} else if (action == 0x0D) { // Take from Cash Inventory
			final Item item = cs.findByCashId(slea.readInt());
			if (item == null) {
				return;
			}
			if (chr.getInventory(
					MapleItemInformationProvider.getInstance()
							.getInventoryType(item.getItemId())).addItem(item) != -1) {
			...
			slea.skip(4);
			final MapleInventory mi = chr.getInventory(MapleInventoryType
					.getByType(slea.readByte()));
			final Item item = mi.findByCashId(cashId);
			if (item == null) {
				return;
			}
			cs.addToInventory(item);
			mi.removeSlot(item.getPosition());
			c.announce(MaplePacketCreator.putIntoCashInventory(item,
					c.getAccID()));
			...
			if (checkBirthday(c, slea.readInt())) {
				...
				final CashItem ring = CashItemFactory.getItem(SN);
				final MapleCharacter partner = c.getChannelServer()
						.getPlayerStorage().getCharacterByName(recipient);
				if (partner == null) {
					chr.getClient()
							.announce(
									MaplePacketCreator
					...
					final int ringid = MapleRing.createRing(ring.getItemId(),
							chr, partner);
					item.setRingId(ringid);
					cs.addToInventory(item);
					c.announce(MaplePacketCreator.showBoughtCashItem(item,
							c.getAccID()));
					cs.gift(partner.getId(), chr.getName(), text, item.getSN(),
							(ringid + 1));
			...
			final int itemId = CashItemFactory.getItem(slea.readInt())
					.getItemId();
			if (chr.getMeso() > 0) {
			...
			if (checkBirthday(c, slea.readInt())) {
				...
				final CashItem ring = CashItemFactory.getItem(snID);
				final String sentTo = slea.readMapleAsciiString();
				final int available = slea.readShort() - 1;
				final String text = slea.readAsciiString(available);
				slea.readByte();
				final MapleCharacter partner = c.getChannelServer()
						.getPlayerStorage().getCharacterByName(sentTo);
				if (partner == null) {
