...
public final class BBSOperationHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		case 0:
			...
			final String title = this.correctLength(
					slea.readMapleAsciiString(), 25);
			String text = this.correctLength(slea.readMapleAsciiString(), 600);
			final int icon = slea.readInt();
			if ((icon >= 0x64) && (icon <= 0x6a)) {
				if (c.getPlayer().getItemQuantity((5290000 + icon) - 0x64,
						false) > 0) {
					return;
				}
		...
		case 4: // reply
			localthreadid = slea.readInt();
			text = this.correctLength(slea.readMapleAsciiString(), 25);
	...
	private static void newBBSReply(MapleClient c, int localthreadid,
			String text) {
	...
	private static void editBBSThread(MapleClient client, String title,
			String text, int icon, int localthreadid) {
	...
	private static void newBBSThread(MapleClient client, String title,
			String text, int icon, boolean bNotice) {
		final MapleCharacter c = client.getPlayer();
		if (c.getGuildId() <= 0) {
			return;
		}
	...
	public static void deleteBBSThread(MapleClient client, int localthreadid) {
		...
		try {
			PreparedStatement ps = con
					.prepareStatement("SELECT threadid, postercid FROM bbs_threads WHERE guildid = ? AND localthreadid = ?");
	...
	public static void deleteBBSReply(MapleClient client, int replyid) {
		...
		try {
			...
			if ((mc.getId() != rs.getInt("postercid"))
					&& (mc.getGuildRank() > 2)) {
				rs.close();
				ps.close();
				return;
	...
	public static void displayThread(MapleClient client, int threadid,
		...
		try {
			...
			try (PreparedStatement ps = con
					.prepareStatement("SELECT * FROM bbs_threads WHERE guildid = ? AND "
				...
				if (threadRS.getInt("replycount") >= 0) {
					ps2 = con
							.prepareStatement("SELECT * FROM bbs_replies WHERE threadid = ?");
					ps2.setInt(
							1,
							!bIsThreadIdLocal ? threadid : threadRS
									.getInt("threadid"));
					repliesRS = ps2.executeQuery();
				}
				client.announce(MaplePacketCreator.showThread(
						bIsThreadIdLocal ? threadid : threadRS
								.getInt("localthreadid"), threadRS, repliesRS));
				repliesRS.close();
			}
			if (ps2 != null) {
				ps2.close();
			...
			se.printStackTrace();
		} catch (final RuntimeException re) {// btw we get this everytime for
												// some
