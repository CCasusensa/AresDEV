...
public final class CouponCodeHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		int type = -1;
		int item = -1;
		validcode = this.getNXCodeValid(code.toUpperCase(), validcode);
		if (validcode) {
			type = this.getNXCode(code, "type");
			item = this.getNXCode(code, "item");
			if (type != 5) {
				try {
					final Connection con = DatabaseConnection.getConnection();
					PreparedStatement ps = con
							.prepareStatement("UPDATE nxcode SET `valid` = 0 WHERE code = "
									+ code);
					ps.executeUpdate();
					ps.close();
