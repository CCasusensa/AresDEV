...
public class PartySearchStartHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		final Collection<MapleMapObject> mapobjs = map.getAllPlayer();
		for (final MapleMapObject mapobj : mapobjs) {
			if (chr.getParty().getMembers().size() > 5) {
				break;
			}
			if (mapobj instanceof MapleCharacter) {
				final MapleCharacter tchar = (MapleCharacter) mapobj;
				final int charlvl = tchar.getLevel();
				if ((charlvl >= min) && (charlvl <= max)
						&& isValidJob(tchar.getJob(), jobs)) {
