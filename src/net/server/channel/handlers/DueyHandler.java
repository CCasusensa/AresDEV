...
public final class DueyHandler extends AbstractMaplePacketHandler {
	private enum Actions {
		TOSERVER_SEND_ITEM(0x02), TOSERVER_CLAIM_PACKAGE(0x04), TOSERVER_REMOVE_PACKAGE(
				0x05), TOSERVER_CLOSE_DUEY(0x07), TOCLIENT_OPEN_DUEY(0x08), TOCLIENT_NOT_ENOUGH_MESOS(
				0x0A), TOCLIENT_NAME_DOES_NOT_EXIST(0x0C), TOCLIENT_SAMEACC_ERROR(
				0x0D), TOCLIENT_SUCCESSFULLY_SENT(0x12), TOCLIENT_SUCCESSFUL_MSG(
				0x17), TOCLIENT_PACKAGE_MSG(0x1B); // Ending byte; 4 if
													// recieved. 3 if delete.
		final byte code;
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if (operation == Actions.TOSERVER_SEND_ITEM.getCode()) {
			...
			if (c.getPlayer().getMeso() >= finalcost) {
				final int accid = getAccIdFromCNAME(recipient, true);
				if (accid != -1) {
					if (accid != c.getAccID()) {
						c.getPlayer().gainMeso(-finalcost, false);
						c.announce(MaplePacketCreator
								.sendDueyMSG(Actions.TOCLIENT_SUCCESSFULLY_SENT
										.getCode()));
						send = true;
					} else {
						c.announce(MaplePacketCreator
								.sendDueyMSG(Actions.TOCLIENT_SAMEACC_ERROR
										.getCode()));
					}
				} else {
					c.announce(MaplePacketCreator
			...
			final int channel = c.getWorldServer().find(recipient);
			if (channel > -1) {
				recipientOn = true;
				final Channel rcserv = c.getWorldServer().getChannel(channel);
				rClient = rcserv.getPlayerStorage()
						.getCharacterByName(recipient).getClient();
			}
			if (send) {
				if (inventId > 0) {
					final MapleInventoryType inv = MapleInventoryType
							.getByType(inventId);
					final Item item = c.getPlayer().getInventory(inv)
							.getItem((byte) itemPos);
					if ((item != null)
							&& (c.getPlayer().getItemQuantity(item.getItemId(),
									false) > amount)) {
						if (ItemConstants.isRechargable(item.getItemId())) {
							MapleInventoryManipulator.removeFromSlot(c, inv,
									(byte) itemPos, item.getQuantity(), true);
						} else {
							MapleInventoryManipulator.removeFromSlot(c, inv,
									(byte) itemPos, amount, true, false);
						}
						this.addItemToDB(item, amount, mesos, c.getPlayer()
								.getName(), getAccIdFromCNAME(recipient, false));
					} else {
						return;
					}
				} else {
					this.addMesoToDB(mesos, c.getPlayer().getName(),
							getAccIdFromCNAME(recipient, false));
				}
				if (recipientOn && (rClient != null)) {
					rClient.announce(MaplePacketCreator
							.sendDueyMSG(Actions.TOCLIENT_PACKAGE_MSG.getCode()));
				}
				c.getPlayer().gainMeso(-fee, false);
			}
		} else if (operation == Actions.TOSERVER_REMOVE_PACKAGE.getCode()) {
			final int packageid = slea.readInt();
			this.removeItemFromDB(packageid);
			c.announce(MaplePacketCreator.removeItemFromDuey(true, packageid));
		} else if (operation == Actions.TOSERVER_CLAIM_PACKAGE.getCode()) {
			final int packageid = slea.readInt();
			final List<DueyPackages> packages = new LinkedList<>();
			DueyPackages dp = null;
			final Connection con = DatabaseConnection.getConnection();
			try {
				DueyPackages dueypack;
				try (PreparedStatement ps = con
						.prepareStatement("SELECT * FROM dueypackages LEFT JOIN dueyitems USING (PackageId) WHERE PackageId = ?")) {
			...
			if (dp.getItem() != null) {
				if (!MapleInventoryManipulator.checkSpace(c, dp.getItem()
						.getItemId(), dp.getItem().getQuantity(), dp.getItem()
						.getOwner())) {
			...
			final int gainmesos = 0;
			final long totalmesos = (long) dp.getMesos()
					+ (long) c.getPlayer().getMeso();
	...
	private void addMesoToDB(int mesos, String sName, int recipientID) {
		this.addItemToDB(null, 1, mesos, sName, recipientID);
	}

	private void addItemToDB(Item item, int quantity, int mesos, String sName,
			int recipientID) {
		final Connection con = DatabaseConnection.getConnection();
		try {
			try (PreparedStatement ps = con
					.prepareStatement("INSERT INTO dueypackages (RecieverId, SenderName, Mesos, TimeStamp, Checked, Type) VALUES (?, ?, ?, ?, ?, ?)")) {
				ps.setInt(1, recipientID);
				ps.setString(2, sName);
				ps.setInt(3, mesos);
				ps.setString(4, this.getCurrentDate());
				ps.setInt(5, 1);
				if (item == null) {
					ps.setInt(6, 3);
					ps.executeUpdate();
					...
					try (ResultSet rs = ps.getGeneratedKeys()) {
						...
						if (item.getType() == 1) { // equips
							ps2 = con
									.prepareStatement("INSERT INTO dueyitems (PackageId, itemid, quantity, upgradeslots, level, str, dex, `int`, luk, hp, mp, watk, matk, wdef, mdef, acc, avoid, hands, speed, jump, owner) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
							...
							ps2 = con
									.prepareStatement("INSERT INTO dueyitems (PackageId, itemid, quantity, owner) VALUES (?, ?, ?, ?)");
	...
	public static List<DueyPackages> loadItems(MapleCharacter chr) {
		...
		try {
			try (PreparedStatement ps = con
					.prepareStatement("SELECT * FROM dueypackages LEFT JOIN dueyitems USING (PackageId) WHERE RecieverId = ?")) {
	...
	private String getCurrentDate() {
		...
		final int month = cal.get(Calendar.MONTH) + 1; // its an array of
														// months.
	...
	private void removeItemFromDB(int packageid) {
		...
		try {
			PreparedStatement ps = con
					.prepareStatement("DELETE FROM dueypackages WHERE PackageId = ?");
			ps.setInt(1, packageid);
			ps.executeUpdate();
			ps.close();
	...
	private static DueyPackages getItemByPID(ResultSet rs) {
		try {
			...
			if (rs.getInt("type") == 1) {
				...
				final Item newItem = new Item(rs.getInt("itemid"), (byte) 0,
