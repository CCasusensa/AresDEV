...
public final class ScrollHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		final MapleItemInformationProvider ii = MapleItemInformationProvider
				.getInstance();
		Equip toScroll = (Equip) c.getPlayer()
				.getInventory(MapleInventoryType.EQUIPPED).getItem(dst);
		final Skill LegendarySpirit = SkillFactory.getSkill(1003);
		if ((c.getPlayer().getSkillLevel(LegendarySpirit) > 0) && (dst >= 0)) {
			legendarySpirit = true;
			toScroll = (Equip) c.getPlayer()
		...
		final MapleInventory useInventory = c.getPlayer().getInventory(
		...
		if ((scrollReqs.size() > 0)
				&& !scrollReqs.contains(toScroll.getItemId())) {
			c.announce(MaplePacketCreator.getInventoryFull());
			return;
		}
		if (whiteScroll) {
		...
		if ((scroll.getItemId() != 2049100)
				&& !this.isCleanSlate(scroll.getItemId())) {
			if (!this.canScroll(scroll.getItemId(), toScroll.getItemId())) {
				return;
			}
		}
		...
		if ((dst < 0)
				&& ((scrollSuccess == Equip.ScrollResult.SUCCESS) || (scrollSuccess == Equip.ScrollResult.CURSE))) {
