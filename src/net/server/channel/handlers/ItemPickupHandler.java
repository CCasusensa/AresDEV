...
public final class ItemPickupHandler extends AbstractMaplePacketHandler {
	...
	@Override
	public final void handlePacket(final SeekableLittleEndianAccessor slea,
			final MapleClient c) {
		slea.readInt(); // Timestamp
		slea.readByte();
		final Point cpos = slea.readPos();
		...
		if (chr.getInventory(
				MapleItemInformationProvider.getInstance().getInventoryType(
						ob.getObjectId())).getNextFreeSlot() > -1) {
			if ((chr.getMapId() > 209000000) && (chr.getMapId() < 209000016)) {// happyville
																				// trees
				final MapleMapItem mapitem = (MapleMapItem) ob;
				if (mapitem.getDropper().getObjectId() == c.getPlayer()
						.getObjectId()) {
					if (MapleInventoryManipulator.addFromDrop(c,
							mapitem.getItem(), false)) {
						chr.getMap().broadcastMessage(
								MaplePacketCreator.removeItemFromMap(
										mapitem.getObjectId(), 2, chr.getId()),
								mapitem.getPosition());
			...
			if (ob instanceof MapleMapItem) {
				...
				synchronized (mapitem) {
					if ((mapitem.getQuest() > 0)
							&& !chr.needQuestItem(mapitem.getQuest(),
									mapitem.getItemId())) {
					...
					final double Distance = cpos.distanceSq(mapitem
							.getPosition());
					if (Distance > 2500) {
						AutobanFactory.SHORT_ITEM_VAC.autoban(chr,
								cpos.toString() + Distance);
					} else if (chr.getPosition().distanceSq(
							mapitem.getPosition()) > 90000.0) {
						AutobanFactory.ITEM_VAC.autoban(chr, cpos.toString()
								+ Distance);
					}
					if (mapitem.getMeso() > 0) {
							...
							for (final MaplePartyCharacter partymem : chr
								...
								if (partymem.isOnline()
									...
									if (somecharacter != null) {
										somecharacter.gainMeso(mesosamm
												/ partynum, true, true, false);
									}
								}
							}
						} else {
						...
						final MapleItemInformationProvider ii = MapleItemInformationProvider
								.getInstance();
						final scriptedItem info = ii
								.getScriptedItemInfo(mapitem.getItem()
										.getItemId());
						if (info.runOnPickup()) {
						...
						chr.getMap().broadcastMessage(
								MaplePacketCreator.updateAriantPQRanking(
										chr.getName(),
										chr.getItemQuantity(4031868, false),
										false));
					} else {
						c.announce(MaplePacketCreator.enableActions());
						return;
