...
public final class PetFoodHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		final AutobanManager abm = chr.getAutobanManager();
		...
		final byte pos = (byte) slea.readShort();
		final int itemId = slea.readInt();
		final Item use = chr.getInventory(MapleInventoryType.USE).getItem(pos);
		if ((use == null) || ((itemId / 10000) != 212)
				|| (use.getItemId() != itemId)) {
			return;
		}
		...
		if (pet.getFullness() < 100) {
			...
			if (gainCloseness && (pet.getCloseness() < 30000)) {
				...
				if (newCloseness >= ExpTable.getClosenessNeededForLevel(pet
					...
					c.announce(MaplePacketCreator.showOwnPetLevelUp(chr
							.getPetIndex(pet)));
					chr.getMap().broadcastMessage(
							MaplePacketCreator.showPetLevelUp(c.getPlayer(),
									chr.getPetIndex(pet)));
				}
			}
			chr.getMap().broadcastMessage(
					MaplePacketCreator.commandResponse(chr.getId(), slot, 1,
							true));
		} else {
			if (gainCloseness) {
				int newCloseness = pet.getCloseness() - 1;
				...
				pet.setCloseness(newCloseness);
				if ((pet.getLevel() > 1)
						&& (newCloseness < ExpTable
								.getClosenessNeededForLevel(pet.getLevel()))) {
					pet.setLevel((byte) (pet.getLevel() - 1));
				}
			}
			chr.getMap().broadcastMessage(
					MaplePacketCreator.commandResponse(chr.getId(), slot, 0,
							false));
		}
		MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE,
				pos, (short) 1, false);
		final Item petz = chr.getInventory(MapleInventoryType.CASH).getItem(
				pet.getPosition());
