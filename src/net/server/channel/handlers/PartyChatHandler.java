...
public final class PartyChatHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if (type == 0) {
			world.buddyChat(recipients, player.getId(), player.getName(),
					chattext);
		} else if ((type == 1) && (player.getParty() != null)) {
			world.partyChat(player.getParty(), chattext, player.getName());
		} else if ((type == 2) && (player.getGuildId() > 0)) {
			Server.getInstance().guildChat(player.getGuildId(),
					player.getName(), player.getId(), chattext);
		} else if ((type == 3) && (player.getGuild() != null)) {
			final int allianceId = player.getGuild().getAllianceId();
			if (allianceId > 0) {
				Server.getInstance().allianceMessage(
						allianceId,
						MaplePacketCreator.multiChat(player.getName(),
								chattext, 3), player.getId(), -1);
			}
		}
	}
}
