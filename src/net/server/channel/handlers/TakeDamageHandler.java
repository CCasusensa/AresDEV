...
public final class TakeDamageHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		slea.readInt();
		final byte damagefrom = slea.readByte();
		slea.readByte(); // Element
		int damage = slea.readInt();
		int oid = 0, monsteridfrom = 0;
		final int pgmr = 0;
		int direction = 0;
		final int pos_x = 0, pos_y = 0;
		int fake = 0;
		final boolean is_pgmr = false, is_pg = true;
		int mpattack = 0;
		...
		if (damagefrom != -3) {
			...
			if (attacker != null) {
				...
				if (damage > 0) {
					loseItems = map.getMonsterById(monsteridfrom).getStats()
							.loseItem();
					if (loseItems != null) {
						MapleInventoryType type;
						final int playerpos = player.getPosition().x;
						byte d = 1;
						final Point pos = new Point(0, player.getPosition().y);
						for (final loseItem loseItem : loseItems) {
							type = MapleItemInformationProvider.getInstance()
									.getInventoryType(loseItem.getId());
							for (byte b = 0; b < loseItem.getX(); b++) {// LOL?
								if (Randomizer.nextInt(101) >= loseItem
										.getChance()) {
									if (player.haveItem(loseItem.getId())) {
										...
										MapleInventoryManipulator.removeById(c,
												type, loseItem.getId(), 1,
												false, false);
										map.spawnItemDrop(c.getPlayer(), c
												.getPlayer(),
												new Item(loseItem.getId(),
														(byte) 0, (short) 1),
												map.calcDropPos(pos,
														player.getPosition()),
												true, true);
		...
		if ((damagefrom != -1) && (damagefrom != -2) && (attacker != null)) {
			...
			final MobSkill skill = MobSkillFactory.getMobSkill(
			...
			if (attacker != null) {
				...
				if ((player.getBuffedValue(MapleBuffStat.MANA_REFLECTION) != null)
						&& (damage > 0) && !attacker.isBoss()) {
					final int jobid = player.getJob().getId();
					if ((jobid == 212) || (jobid == 222) || (jobid == 232)) {
						final int id = (jobid * 10000) + 2;
						final Skill manaReflectSkill = SkillFactory
								.getSkill(id);
						if (player.isBuffFrom(MapleBuffStat.MANA_REFLECTION,
								manaReflectSkill)
								&& (player.getSkillLevel(manaReflectSkill) > 0)
								&& manaReflectSkill.getEffect(
										player.getSkillLevel(manaReflectSkill))
										.makeChanceResult()) {
							int bouncedamage = ((damage * manaReflectSkill
									.getEffect(
											player.getSkillLevel(manaReflectSkill))
									.getX()) / 100);
							...
							map.broadcastMessage(player, MaplePacketCreator
									.damageMonster(oid, bouncedamage), true);
							player.getClient()
									.announce(
											MaplePacketCreator
													.showOwnBuffEffect(id, 5));
							map.broadcastMessage(
									player,
									MaplePacketCreator.showBuffeffect(
											player.getId(), id, 5), false);
						}
					}
				}
			}
		...
		if ((damage > 0) && !player.isHidden()) {
			if ((attacker != null) && !attacker.isBoss()) {
				if ((damagefrom == -1)
					...
					int bouncedamage = (int) (damage * (player.getBuffedValue(
							MapleBuffStat.POWERGUARD).doubleValue() / 100));
					bouncedamage = Math.min(bouncedamage,
							attacker.getMaxHp() / 10);
					map.damageMonster(player, attacker, bouncedamage);
					damage -= bouncedamage;
					map.broadcastMessage(
							player,
			...
			if (damagefrom != -3) {
				...
				if ((jobid < 200) && ((jobid % 10) == 2)) {
					achilles1 = SkillFactory
							.getSkill(((jobid * 10000) + jobid) == 112 ? 4 : 5);
			...
			final Integer mesoguard = player
					.getBuffedValue(MapleBuffStat.MESOGUARD);
			if ((player.getBuffedValue(MapleBuffStat.MAGIC_GUARD) != null)
					&& (mpattack == 0)) {
				int mploss = (int) (damage * (player.getBuffedValue(
						MapleBuffStat.MAGIC_GUARD).doubleValue() / 100.0));
				...
				final int mesoloss = (int) (damage * (mesoguard.doubleValue() / 100.0));
				...
				if (player.getBuffedValue(MapleBuffStat.MONSTER_RIDING) != null) {
					if (player.getBuffedValue(MapleBuffStat.MONSTER_RIDING)
							.intValue() == Corsair.BATTLE_SHIP) {
		...
		if (!player.isHidden()) {
			map.broadcastMessage(player, MaplePacketCreator.damagePlayer(
					damagefrom, monsteridfrom, player.getId(), damage, fake,
					direction, is_pgmr, pgmr, is_pg, oid, pos_x, pos_y), false);
			player.checkBerserk();
		}
		if ((map.getId() >= 925020000) && (map.getId() < 925030000)) {
			player.setDojoEnergy(player.isGM() ? 300
					: player.getDojoEnergy() < 300 ? player.getDojoEnergy() + 1
							: 0); // Fking gm's
			player.getClient().announce(
					MaplePacketCreator.getEnergy("energy",
							player.getDojoEnergy()));
		}
	}
}
