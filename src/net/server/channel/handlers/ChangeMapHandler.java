...
public final class ChangeMapHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if (slea.available() == 0) { // Cash Shop :)
			...
			try {
				c.announce(MaplePacketCreator.getChannelChange(
			...
			try {
				...
				final MaplePortal portal = chr.getMap().getPortal(startwp);
				slea.readByte();
				final boolean wheel = slea.readShort() > 0;
				if ((targetid != -1) && !chr.isAlive()) {
					boolean executeStandardPath = true;
					if (chr.getEventInstance() != null) {
						executeStandardPath = chr.getEventInstance()
								.revivePlayer(chr);
					}
					if (executeStandardPath) {
						...
						if (wheel && (chr.getItemQuantity(5510000, false) > 0)) {
							MapleInventoryManipulator.removeById(c,
									...
									false);
							chr.announce(MaplePacketCreator.showWheelsLeft(chr
									.getItemQuantity(5510000, false)));
						} else {
					...
					final MapleMap to = c.getChannelServer().getMapFactory()
							.getMap(targetid);
					chr.changeMap(to, to.getPortal(0));
				} else if ((targetid != -1) && !chr.isGM()) {// Thanks celino
																// for
																// saving me
																// some
																// time (:
					...
					if (divi == 0) {
						...
						if (targetid == 104000000) {
							...
							warp = true;
													...
													// or else = hack
						if ((targetid == 130000000)
								|| ((targetid / 100) == 9130401)) { // Cygnus
							// introduction
							warp = true;
						}
					...
					if (warp) {
						final MapleMap to = c.getChannelServer()
								.getMapFactory().getMap(targetid);
						chr.changeMap(to, to.getPortal(0));
					}
				}
