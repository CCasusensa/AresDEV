	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if (this.isFinisher(attack.skill)) {
			...
			if ((attack.skill != 1111008) && (comboBuff != null)) {
				final int orbcount = player.getBuffedValue(MapleBuffStat.COMBO);
				final int oid = player.isCygnus() ? DawnWarrior.COMBO
						: Crusader.COMBO;
				final int advcomboid = player.isCygnus() ? DawnWarrior.ADVANCED_COMBO
						: Hero.ADVANCED_COMBO;
				final Skill combo = SkillFactory.getSkill(oid);
				final Skill advcombo = SkillFactory.getSkill(advcomboid);
				MapleStatEffect ceffect;
				final int advComboSkillLevel = player.getSkillLevel(advcombo);
				...
				if (orbcount < (ceffect.getX() + 1)) {
					...
					int duration = combo.getEffect(player.getSkillLevel(oid))
							.getDuration();
					final List<Pair<MapleBuffStat, Integer>> stat = Collections
							.singletonList(new Pair<>(MapleBuffStat.COMBO,
									neworbcount));
					player.setBuffedValue(MapleBuffStat.COMBO, neworbcount);
					duration -= (int) (System.currentTimeMillis() - player
							.getBuffedStarttime(MapleBuffStat.COMBO));
					c.announce(MaplePacketCreator.giveBuff(oid, duration, stat));
					player.getMap().broadcastMessage(
							player,
							MaplePacketCreator.giveForeignBuff(player.getId(),
									stat), false);
				}
			} else if ((player.getSkillLevel(player.isCygnus() ? SkillFactory
					.getSkill(15100004) : SkillFactory.getSkill(5110001)) > 0)
					&& (player.getJob().isA(MapleJob.MARAUDER) || player
							.getJob().isA(MapleJob.THUNDERBREAKER2))) {
		...
		if ((attack.numAttacked > 0)
				&& (attack.skill == DragonKnight.SACRIFICE)) {
			int totDamageToOneMonster = 0; // sacrifice attacks only 1 mob with
											// 1 attack
			final Iterator<List<Integer>> dmgIt = attack.allDamage.values()
					.iterator();
			if (dmgIt.hasNext()) {
				totDamageToOneMonster = dmgIt.next().get(0).intValue();
			}
			final int remainingHP = player.getHp()
					- ((totDamageToOneMonster * attack.getAttackEffect(player,
							null).getX()) / 100);
			if (remainingHP > 1) {
		...
		if ((attack.numAttacked > 0) && (attack.skill == 1211002)) {
			...
			final int advcharge_level = player.getSkillLevel(SkillFactory
					.getSkill(1220010));
			if (advcharge_level > 0) {
				advcharge_prob = SkillFactory.getSkill(1220010)
						.getEffect(advcharge_level).makeChanceResult();
			}
			if (!advcharge_prob) {
		...
		if ((numFinisherOrbs == 0) && this.isFinisher(attack.skill)) {
			return;
		}
		if (attack.skill > 0) {
		...
		if (((player.getSkillLevel(SkillFactory.getSkill(NightWalker.VANISH)) > 0)
				|| (player.getSkillLevel(SkillFactory
						.getSkill(WindArcher.WIND_WALK)) > 0) || (player
				.getSkillLevel(SkillFactory.getSkill(Rogue.DARK_SIGHT)) > 0))
		...
		this.applyAttack(attack, player, attackCount);
	}
}