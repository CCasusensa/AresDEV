...
public final class PlayerInteractionHandler extends AbstractMaplePacketHandler {
	public enum Action {
		CREATE(0), INVITE(2), DECLINE(3), VISIT(4), ROOM(5), CHAT(6), CHAT_THING(
				8), EXIT(0xA), OPEN(0xB), TRADE_BIRTHDAY(0x0E), SET_ITEMS(0xF), SET_MESO(
				0x10), CONFIRM(0x11), TRANSACTION(0x14), ADD_ITEM(0x16), BUY(
				0x17), UPDATE_MERCHANT(0x19), REMOVE_ITEM(0x1B), BAN_PLAYER(
				0x1C), MERCHANT_THING(0x1D), OPEN_STORE(0x1E), PUT_ITEM(0x21), MERCHANT_BUY(
				0x22), TAKE_ITEM_BACK(0x26), MAINTENANCE_OFF(0x27), MERCHANT_ORGANIZE(
				0x28), CLOSE_MERCHANT(0x29), REAL_CLOSE_MERCHANT(0x2A), MERCHANT_MESO(
				0x2B), SOMETHING(0x2D), VIEW_VISITORS(0x2E), BLACKLIST(0x2F), REQUEST_TIE(
				0x32), ANSWER_TIE(0x33), GIVE_UP(0x34), EXIT_AFTER_GAME(0x38), CANCEL_EXIT(
				0x39), READY(0x3A), UN_READY(0x3B), START(0x3D), GET_RESULT(
				0x3E), SKIP(0x3F), MOVE_OMOK(0x40), SELECT_CARD(0x44);
		final byte code;
		...
		if (mode == Action.CREATE.getCode()) {
			...
			if (createType == 3) {// trade
				...
				if ((chr.getChalkboard() != null)
						|| FieldLimit.CANNOTMINIGAME.check(chr.getMap()
								.getFieldLimit())) {
					return;
				}
				final String desc = slea.readMapleAsciiString();
				slea.readByte(); // 20 6E 4E
				final int type = slea.readByte(); // 20 6E 4E
				final MapleMiniGame game = new MapleMiniGame(chr, desc);
				chr.setMiniGame(game);
				game.setPieceType(type);
				game.setGameType("omok");
				chr.getMap().addMapObject(game);
				chr.getMap().broadcastMessage(
						MaplePacketCreator.addOmokBox(chr, 1, 0));
				...
				chr.getMap().addMapObject(game);
				chr.getMap().broadcastMessage(
						MaplePacketCreator.addMatchCardBox(chr, 1, 0));
				...
				if (((chr.getMapId() > 910000000) && (chr.getMapId() < 910000023))
					...
					if (createType == 4) {
						final MaplePlayerShop shop = new MaplePlayerShop(
								c.getPlayer(), desc);
						chr.setPlayerShop(shop);
						chr.getMap().addMapObject(shop);
						shop.sendShop(c);
						c.announce(MaplePacketCreator
								.getPlayerShopRemoveVisitor(1));
					} else {
						final HiredMerchant merchant = new HiredMerchant(chr,
								itemId, desc);
						chr.setHiredMerchant(merchant);
						chr.getClient().getChannelServer()
								.addHiredMerchant(chr.getId(), merchant);
						chr.announce(MaplePacketCreator.getHiredMerchant(chr,
								merchant, true));
					}
				}
			}
			...
			MapleTrade.inviteTrade(chr,
					chr.getMap().getCharacterById(otherPlayer));
			...
			if ((chr.getTrade() != null)
				...
				MapleTrade
						.visitTrade(chr, chr.getTrade().getPartner().getChr());
			} else {
				final int oid = slea.readInt();
				final MapleMapObject ob = chr.getMap().getMapObject(oid);
				if (ob instanceof MaplePlayerShop) {
					final MaplePlayerShop shop = (MaplePlayerShop) ob;
					...
					final MapleMiniGame game = (MapleMiniGame) ob;
					if (game.hasFreeSlot() && !game.isVisitor(c.getPlayer())) {
						...
						chr.getClient().announce(
								MaplePacketCreator.getMiniGameFull());
					}
				} else if ((ob instanceof HiredMerchant)
						&& (chr.getHiredMerchant() == null)) {
					final HiredMerchant merchant = (HiredMerchant) ob;
					if (merchant.isOwner(c.getPlayer())) {
						merchant.setOpen(false);
						merchant.removeAllVisitors("");
						c.announce(MaplePacketCreator.getHiredMerchant(chr,
								merchant, false));
						...
						c.announce(MaplePacketCreator.getHiredMerchant(
			...
			final HiredMerchant merchant = chr.getHiredMerchant();
			if (chr.getTrade() != null) {
				...
				final MapleMiniGame game = chr.getMiniGame();
				if (game != null) {
					game.chat(c, slea.readMapleAsciiString());
				}
			} else if (merchant != null) {
				final String message = chr.getName() + " : "
						+ slea.readMapleAsciiString();
				final byte slot = (byte) (merchant
						.getVisitorSlot(c.getPlayer()) + 1);
				merchant.getMessages().add(new Pair<>(message, slot));
				merchant.broadcastToVisitors(MaplePacketCreator
						.hiredMerchantChat(message, slot));
			}
			...
			if (chr.getTrade() != null) {
				...
				if (shop != null) {
					if (shop.isOwner(c.getPlayer())) {
						for (final MaplePlayerShopItem mpsi : shop.getItems()) {
							if (mpsi.getBundles() > 2) {
								final Item iItem = mpsi.getItem().copy();
								iItem.setQuantity((short) (mpsi.getBundles() * iItem
										.getQuantity()));
						...
						chr.getMap()
								.broadcastMessage(
										MaplePacketCreator.removeCharBox(c
												.getPlayer()));
						shop.removeVisitors();
					} else {
			...
			if ((shop != null) && shop.isOwner(c.getPlayer())) {
				slea.readByte();// 01
				chr.getMap().broadcastMessage(
						MaplePacketCreator.addCharBox(c.getPlayer(), 4));
				...
				chr.setHiredMerchant(null);
				chr.getMap().broadcastMessage(
						MaplePacketCreator.spawnHiredMerchant(merchant));
				slea.readByte();
			}
			...
			if (game.getGameType().equals("omok")) {
				game.broadcast(MaplePacketCreator.getMiniGameStart(game,
						game.getLoser()));
				chr.getMap().broadcastMessage(
						MaplePacketCreator.addOmokBox(game.getOwner(), 2, 1));
			}
			if (game.getGameType().equals("matchcard")) {
				game.shuffleList();
				game.broadcast(MaplePacketCreator.getMatchCardStart(game,
						game.getLoser()));
				chr.getMap().broadcastMessage(
						MaplePacketCreator.addMatchCardBox(game.getOwner(), 2,
								1));
			}
			...
			if (game.getGameType().equals("omok")) {
				if (game.isOwner(c.getPlayer())) {
					game.broadcast(MaplePacketCreator
							.getMiniGameOwnerForfeit(game));
				} else {
					game.broadcast(MaplePacketCreator
							.getMiniGameVisitorForfeit(game));
				}
			}
			if (game.getGameType().equals("matchcard")) {
				if (game.isOwner(c.getPlayer())) {
					game.broadcast(MaplePacketCreator
							.getMatchCardVisitorWin(game));
				} else {
					game.broadcast(MaplePacketCreator
							.getMatchCardOwnerWin(game));
				}
			}
			...
			if (game.isOwner(c.getPlayer())) {
				game.broadcastToVisitor(MaplePacketCreator
						.getMiniGameRequestTie(game));
			} else {
				game.getOwner()
						.getClient()
						.announce(
								MaplePacketCreator.getMiniGameRequestTie(game));
			}
			...
			final int type = slea.readByte(); // piece ( 1 or 2; Owner has one
												// piece,
			...
			if (turn == 1) {
				...
				if (game.isOwner(c.getPlayer())) {
					game.broadcastToVisitor(MaplePacketCreator
							.getMatchCardSelect(game, turn, slot, firstslot,
									turn));
				} else {
					game.getOwner()
							.getClient()
							.announce(
									MaplePacketCreator.getMatchCardSelect(game,
											turn, slot, firstslot, turn));
				}
			} else if ((game.getCardId(firstslot + 1)) == (game
					.getCardId(slot + 1))) {
				if (game.isOwner(c.getPlayer())) {
					game.broadcast(MaplePacketCreator.getMatchCardSelect(game,
							turn, slot, firstslot, 2));
					game.setOwnerPoints();
				} else {
					game.broadcast(MaplePacketCreator.getMatchCardSelect(game,
							turn, slot, firstslot, 3));
					game.setVisitorPoints();
				}
			} else if (game.isOwner(c.getPlayer())) {
				game.broadcast(MaplePacketCreator.getMatchCardSelect(game,
						turn, slot, firstslot, 0));
			} else {
				game.broadcast(MaplePacketCreator.getMatchCardSelect(game,
						turn, slot, firstslot, 1));
			}
			...
			final MapleItemInformationProvider ii = MapleItemInformationProvider
					.getInstance();
			final MapleInventoryType ivType = MapleInventoryType.getByType(slea
					.readByte());
			final Item item = chr.getInventory(ivType).getItem(
					(byte) slea.readShort());
			...
			if (chr.getTrade() != null) {
				if (((quantity <= item.getQuantity()) && (quantity >= 0))
					...
					if (ii.isDropRestricted(item.getItemId())) { // ensure that
																	...
																	// trade
																	// window
						if (!(((item.getFlag() & ItemConstants.KARMA) == ItemConstants.KARMA) || ((item
								.getFlag() & ItemConstants.SPIKES) == ItemConstants.SPIKES))) {
					...
					if (ItemConstants.isRechargable(item.getItemId())) {
						...
						MapleInventoryManipulator.removeFromSlot(c, ivType,
								item.getPosition(), item.getQuantity(), true);
					} else {
						tradeItem.setQuantity(quantity);
						MapleInventoryManipulator.removeFromSlot(c, ivType,
								item.getPosition(), quantity, true);
					}
			...
			MapleTrade.completeTrade(c.getPlayer());
				...
				|| (mode == Action.PUT_ITEM.getCode())) {
			final MapleInventoryType type = MapleInventoryType.getByType(slea
					.readByte());
			final byte slot = (byte) slea.readShort();
			final short bundles = slea.readShort();
			if ((chr.getItemQuantity(chr.getInventory(type).getItem(slot)
					.getItemId(), false) < bundles)
					|| (chr.getInventory(type).getItem(slot).getFlag() == ItemConstants.UNTRADEABLE)) {
				return;
			}
			final short perBundle = slea.readShort();
			final int price = slea.readInt();
			if ((perBundle < 0) || ((perBundle * bundles) > 2000)
					|| (bundles < 0) || (price < 0)) {
				return;
			}
			...
			final MaplePlayerShopItem item = new MaplePlayerShopItem(sellItem,
					bundles, price);
			final MaplePlayerShop shop = chr.getPlayerShop();
			final HiredMerchant merchant = chr.getHiredMerchant();
			if ((shop != null) && shop.isOwner(c.getPlayer())) {
			...
			if ((shop != null) && shop.isOwner(c.getPlayer())) {
				...
				final Item ivItem = item.getItem().copy();
			...
			if (chr.getMerchantMeso() > 0) {
				final int possible = Integer.MAX_VALUE - chr.getMerchantMeso();
				if (possible > 0) {
			...
			c.announce(MaplePacketCreator.updateHiredMerchant(merchant, chr));
				...
				|| (mode == Action.MERCHANT_BUY.getCode())) {
			...
			if ((shop != null) && shop.isVisitor(c.getPlayer())) {
				...
				merchant.broadcastToVisitors(MaplePacketCreator
						.updateHiredMerchant(merchant, c.getPlayer()));
			}
		} else if (mode == Action.TAKE_ITEM_BACK.getCode()) {
			final HiredMerchant merchant = chr.getHiredMerchant();
			if ((merchant != null) && merchant.isOwner(c.getPlayer())) {
				final int slot = slea.readShort();
				final MaplePlayerShopItem item = merchant.getItems().get(slot);
				if (item.getBundles() > 0) {
					final Item iitem = item.getItem();
					iitem.setQuantity((short) (item.getItem().getQuantity() * item
							.getBundles()));
			...
			if ((chr.getPlayerShop() != null)
					&& chr.getPlayerShop().isOwner(c.getPlayer())) {
