...
public final class ItemSortHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if ((inv > 0) && (inv <= 5)) {
			...
			final MapleInventoryType pInvType = MapleInventoryType
					.getByType(inv);
			final MapleInventory pInv = chr.getInventory(pInvType);
			while (!sorted) {
				...
				if (freeSlot != -1) {
					...
					if ((itemSlot <= 96) && (itemSlot >= 1)) {
						MapleInventoryManipulator.move(c, pInvType, itemSlot,
								freeSlot);
					} else {
						sorted = true;
					}
