...
public final class UseSolomonHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		final MapleItemInformationProvider ii = MapleItemInformationProvider
				.getInstance();
		final Item slotItem = c.getPlayer()
				.getInventory(MapleInventoryType.USE).getItem(slot);
		final int gachaexp = ii.getExpById(itemId);
		if ((c.getPlayer().getInventory(MapleInventoryType.USE)
				.countById(itemId) <= 0)
				|| (slotItem.getItemId() != itemId)
				|| (c.getPlayer().getLevel() > ii.getMaxLevelById(itemId))) {
			return;
		}
