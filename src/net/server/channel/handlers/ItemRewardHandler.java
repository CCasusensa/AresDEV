...
public final class ItemRewardHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if ((c.getPlayer().getInventory(MapleInventoryType.USE).getItem(slot)
				.getItemId() != itemId)
				|| (c.getPlayer().getInventory(MapleInventoryType.USE)
						.countById(itemId) < 1)) {
			return;
		}
		final MapleItemInformationProvider ii = MapleItemInformationProvider
				.getInstance();
		final Pair<Integer, List<RewardItem>> rewards = ii
				.getItemReward(itemId);
		for (final RewardItem reward : rewards.getRight()) {
			if (!MapleInventoryManipulator.checkSpace(c, reward.itemid,
					reward.quantity, "")) {
			...
			if (Randomizer.nextInt(rewards.getLeft()) < reward.prob) {// Is it
				...
				if (ItemConstants.getInventoryType(reward.itemid) == MapleInventoryType.EQUIP) {
					...
					MapleInventoryManipulator.addById(c, reward.itemid,
							reward.quantity);
				}
				MapleInventoryManipulator.removeById(c, MapleInventoryType.USE,
						itemId, 1, false, false);
				if (reward.worldmsg != null) {
					final String msg = reward.worldmsg;
					...
					Server.getInstance().broadcastMessage(c.getWorld(),
							MaplePacketCreator.serverNotice(6, msg));
				}
				break;
			}
		}
