...
public final class MonsterCarnivalHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		final int number = slea.readShort();
		if (carnival != null) {
			if ((chr.getCarnivalParty() != carnival.getPartyRed())
					|| (chr.getCarnivalParty() != carnival.getPartyBlue())) {
				chr.getMap().broadcastMessage(MaplePacketCreator.leaveCPQ(chr));
				chr.changeMap(980000010);
			}
			if (chr.getCP() > this.getPrice(tab, number)) {
				if (tab == 0) { // SPAWNING
					if (chr.getCarnivalParty().canSummon()) {
						chr.getMap().spawnCPQMonster(
								MapleLifeFactory.getMonster(this
										.getMonster(number)),
								new Point(1, 1),
								carnival.oppositeTeam(chr.getCarnivalParty())
										.getTeam());
					...
					final int rid = 9980000 + chr.getTeam();
					final MapleReactor reactor = new MapleReactor(
							MapleReactorFactory.getReactor(rid), rid);
					/*
				...
				chr.getMap().broadcastMessage(
						MaplePacketCreator.CPQMessage((byte) 1));
			}
		} else {
