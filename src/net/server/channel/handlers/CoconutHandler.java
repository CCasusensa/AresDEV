...
public final class CoconutHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		final MapleCoconut event = map.getCoconut();
		final MapleCoconuts nut = event.getCoconut(id);
		if (!nut.isHittable()) {
			return;
		}
		if (event == null) {
		...
		if ((nut.getHits() > 2) && (Math.random() < 0.4)) {
			...
			if ((Math.random() < 0.05) && (event.getBombings() > 0)) {
				map.broadcastMessage(MaplePacketCreator
						.hitCoconut(false, id, 2));
				event.bombCoconut();
			} else if (event.getFalling() > 0) {
				map.broadcastMessage(MaplePacketCreator
						.hitCoconut(false, id, 3));
				event.fallCoconut();
				if (c.getPlayer().getTeam() == 0) {
					event.addMapleScore();
					map.broadcastMessage(MaplePacketCreator.serverNotice(5, c
							.getPlayer().getName()
				...
				map.broadcastMessage(MaplePacketCreator.coconutScore(
						event.getMapleScore(), event.getStoryScore()));
			}
		} else {
			nut.hit();
