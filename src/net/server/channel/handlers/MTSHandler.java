...
public final class MTSHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea,
		...
		if (slea.available() > 0) {
			...
			if (op == 2) { // put item up for sale
				...
				final Item i = c.getPlayer().getInventory(type).getItem(slot)
						.copy();
				if ((i != null) && (c.getPlayer().getMeso() >= 5000)) {
					final Connection con = DatabaseConnection.getConnection();
					try {
						PreparedStatement ps = con
								.prepareStatement("SELECT COUNT(*) FROM mts_items WHERE seller = ?");
						...
						if (rs.next()) {
							if (rs.getInt(1) > 10) { // They have more than 10
								...
								c.getPlayer()
										.dropMessage(1,
												"You already have 10 items up for auction!");
								c.announce(getMTS(1, 0, 0));
								c.announce(MaplePacketCreator
										.transferInventory(this.getTransfer(c
												.getPlayer().getId())));
								c.announce(MaplePacketCreator
										.notYetSoldInv(this.getNotYetSold(c
												.getPlayer().getId())));
								rs.close();
								ps.close();
								return;
					...
					c.announce(MaplePacketCreator.transferInventory(this
							.getTransfer(c.getPlayer().getId())));
					c.announce(MaplePacketCreator.notYetSoldInv(this
				...
				if ((tab == 4) && (type == 0)) {
					c.announce(this.getCart(c.getPlayer().getId()));
				} else if ((tab == c.getPlayer().getCurrentTab())
						&& (type == c.getPlayer().getCurrentType())
						&& (c.getPlayer().getSearch() != null)) {
					c.announce(this.getMTSSearch(tab, type, c.getPlayer()
							.getCurrentCI(), c.getPlayer().getSearch(), page));
				} else {
				...
				c.announce(this.getMTSSearch(tab, type, ci, search, c
						.getPlayer().getCurrentPage()));
				...
				try {
					PreparedStatement ps = con
							.prepareStatement("UPDATE mts_items SET transfer = 1 WHERE id = ? AND seller = ?");
					ps.setInt(1, id);
				...
				c.announce(MaplePacketCreator.enableCSUse());
				c.announce(getMTS(c.getPlayer().getCurrentTab(), c.getPlayer()
						.getCurrentType(), c.getPlayer().getCurrentPage()));
				c.announce(MaplePacketCreator.notYetSoldInv(this
						.getNotYetSold(c.getPlayer().getId())));
				c.announce(MaplePacketCreator.transferInventory(this
						.getTransfer(c.getPlayer().getId())));
				...
				try {
					...
					if (rs.next()) {
						Item i;
						if (rs.getInt("type") != 1) {
							final Item ii = new Item(rs.getInt("itemid"),
									(byte) 0, (short) rs.getInt("quantity"));
							...
							final Equip equip = new Equip(rs.getInt("itemid"),
									(byte) rs.getInt("position"), -1);
						...
						try (PreparedStatement pse = con
								.prepareStatement("DELETE FROM mts_items WHERE id = ? AND seller = ? AND transfer = 1")) {
							pse.setInt(1, id);
						...
						c.announce(MaplePacketCreator.enableCSUse());
						c.announce(this.getCart(c.getPlayer().getId()));
						c.announce(getMTS(c.getPlayer().getCurrentTab(), c
								.getPlayer().getCurrentType(), c.getPlayer()
								.getCurrentPage()));
						c.announce(MaplePacketCreator.MTSConfirmTransfer(
								i.getQuantity(), i.getPosition()));
						c.announce(MaplePacketCreator.transferInventory(this
								.getTransfer(c.getPlayer().getId())));
					}
					rs.close();
				...
				try {
					try (PreparedStatement ps1 = con
							.prepareStatement("SELECT * FROM mts_items WHERE id = ? AND seller <> ?")) {
						...
						try (ResultSet rs1 = ps1.executeQuery()) {
							if (rs1.next()) {
								final PreparedStatement ps = con
										.prepareStatement("SELECT * FROM mts_cart WHERE cid = ? AND itemid = ?");
								ps.setInt(1, c.getPlayer().getId());
								ps.setInt(2, id);
								try (ResultSet rs = ps.executeQuery()) {
									if (!rs.next()) {
										try (PreparedStatement pse = con
												.prepareStatement("INSERT INTO mts_cart (cid, itemid) VALUES (?, ?)")) {
				...
				try {
					try (PreparedStatement ps = con
							.prepareStatement("DELETE FROM mts_cart WHERE itemid = ? AND cid = ?")) {
						ps.setInt(1, id);
						ps.setInt(2, c.getPlayer().getId());
						ps.executeUpdate();
					}
				...
				try {
					...
					rs = ps.executeQuery();
					if (rs.next()) {
						final int price = rs.getInt("price") + 100
								+ (int) (rs.getInt("price") * 0.1); // taxes
						if (c.getPlayer().getCashShop().getCash(4) >= price) { // FIX
							boolean alwaysnull = true;
							for (final Channel cserv : Server.getInstance()
									.getAllChannels()) {
								final MapleCharacter victim = cserv
										.getPlayerStorage().getCharacterById(
												rs.getInt("seller"));
								if (victim != null) {
									victim.getCashShop().gainCash(4,
											rs.getInt("price"));
									alwaysnull = false;
								}
							}
							if (alwaysnull) {
								...
								try (PreparedStatement pse = con
									...
									if (rse.next()) {
										try (PreparedStatement psee = con
												.prepareStatement("UPDATE accounts SET nxPrepaid = nxPrepaid + ? WHERE id = ?")) {
											psee.setInt(1, rs.getInt("price"));
											psee.setInt(2,
													rse.getInt("accountid"));
											psee.executeUpdate();
										}
									}
								}
							...
							PreparedStatement pse = con
									.prepareStatement("UPDATE mts_items SET seller = ?, transfer = 1 WHERE id = ?");
							pse.setInt(1, c.getPlayer().getId());
							pse.setInt(2, id);
							pse.executeUpdate();
							pse.close();
							pse = con
									.prepareStatement("DELETE FROM mts_cart WHERE itemid = ?");
							pse.setInt(1, id);
							pse.executeUpdate();
							pse.close();
							...
							c.announce(MaplePacketCreator.enableCSUse());
							c.announce(getMTS(c.getPlayer().getCurrentTab(), c
									.getPlayer().getCurrentType(), c
									.getPlayer().getCurrentPage()));
							c.announce(MaplePacketCreator.MTSConfirmBuy());
							c.announce(MaplePacketCreator.showMTSCash(c
									.getPlayer()));
							c.announce(MaplePacketCreator
									.transferInventory(this.getTransfer(c
											.getPlayer().getId())));
				...
				System.out.println("Unhandled OP(MTS): " + op + " Packet: "
						+ slea.toString());
			}
		} else {
			c.announce(MaplePacketCreator.showMTSCash(c.getPlayer()));
	...
	public List<MTSItemInfo> getNotYetSold(int cid) {
		...
		try {
			...
			while (rs.next()) {
				if (rs.getInt("type") != 1) {
					final Item i = new Item(rs.getInt("itemid"), (byte) 0,
							(short) rs.getInt("quantity"));
					i.setOwner(rs.getString("owner"));
					items.add(new MTSItemInfo(i, rs.getInt("price"), rs
							.getInt("id"), rs.getInt("seller"), rs
							.getString("sellername"), rs.getString("sell_ends")));
				} else {
					final Equip equip = new Equip(rs.getInt("itemid"),
							(byte) rs.getInt("position"), -1);
					equip.setOwner(rs.getString("owner"));
					...
					equip.setLevel((byte) rs.getInt("level"));
					equip.setFlag((byte) rs.getInt("flag"));
					items.add(new MTSItemInfo(equip, rs.getInt("price"), rs
							.getInt("id"), rs.getInt("seller"), rs
							.getString("sellername"), rs.getString("sell_ends")));
				}
			}
	...
	public byte[] getCart(int cid) {
		...
		try {
			...
			while (rs.next()) {
				try (PreparedStatement pse = con
						.prepareStatement("SELECT * FROM mts_items WHERE id = ?")) {
					pse.setInt(1, rs.getInt("itemid"));
					final ResultSet rse = pse.executeQuery();
					if (rse.next()) {
						if (rse.getInt("type") != 1) {
							final Item i = new Item(rse.getInt("itemid"),
									(byte) 0, (short) rse.getInt("quantity"));
							i.setOwner(rse.getString("owner"));
							items.add(new MTSItemInfo(i, rse.getInt("price"),
									rse.getInt("id"), rse.getInt("seller"), rse
											.getString("sellername"), rse
											.getString("sell_ends")));
						} else {
							final Equip equip = new Equip(rse.getInt("itemid"),
									(byte) rse.getInt("position"), -1);
							...
							equip.setUpgradeSlots((byte) rse
									.getInt("upgradeslots"));
							equip.setLevel((byte) rse.getInt("level"));
							equip.setFlag((byte) rs.getInt("flag"));
							items.add(new MTSItemInfo(equip, rse
									.getInt("price"), rse.getInt("id"), rse
									.getInt("seller"), rse
									.getString("sellername"), rse
									.getString("sell_ends")));
						}
	...
	private static byte[] getMTS(int tab, int type, int page) {
		...
		try {
			...
			rs.close();
			ps.close();
			ps = con.prepareStatement("SELECT COUNT(*) FROM mts_items WHERE tab = ? "
					+ (type != 0 ? "AND type = ?" : "") + "AND transfer = 0");
			ps.setInt(1, tab);
			if (type != 0) {
				ps.setInt(2, type);
			}
	...
	public byte[] getMTSSearch(int tab, int type, int cOi, String search,
			int page) {
		final List<MTSItemInfo> items = new ArrayList<>();
		...
		if (cOi != 0) {
			...
			for (final Pair<Integer, String> itemPair : ii.getAllItems()) {
				if (itemPair.getRight().toLowerCase()
		...
		try {
			if (type != 0) {
				ps = con.prepareStatement("SELECT * FROM mts_items WHERE tab = ? "
						+ listaitems
