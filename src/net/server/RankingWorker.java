...
public class RankingWorker implements Runnable {
	...
	public void run() {
		try {
			...
			for (int i = 0; i < 3; i += 2) {
				for (int j = 1; j < 6; j++) {
					this.updateRanking(MapleJob.getById((i * 500) + (100 * j)));
				}
			}
			this.con.commit();
	...
	private void updateRanking(MapleJob job) throws SQLException {
		String sqlCharSelect = "SELECT c.id, "
				+ (job != null ? "c.jobRank, c.jobRankMove"
						: "c.rank, c.rankMove")
		...
		final PreparedStatement charSelect = this.con
				.prepareStatement(sqlCharSelect);
		if (job != null) {
		...
		final PreparedStatement ps = this.con
				.prepareStatement("UPDATE characters SET "
						+ (job != null ? "jobRank = ?, jobRankMove = ? "
								: "rank = ?, rankMove = ? ") + "WHERE id = ?");
		int rank = 0;
		while (rs.next()) {
			int rankMove = 0;
			rank++;
			if ((rs.getLong("lastlogin") < this.lastUpdate)
					|| (rs.getInt("loggedin") > 0)) {
				rankMove = rs
						.getInt((job != null ? "jobRankMove" : "rankMove"));
			}
