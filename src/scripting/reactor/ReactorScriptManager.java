...
public class ReactorScriptManager extends AbstractScriptManager {
	...
	public void act(MapleClient c, MapleReactor reactor) {
		try {
			...
			final Invocable iv = this.getInvocable("reactor/" + reactor.getId()
					+ ".js", c);
		...
		} catch (final ScriptException | NoSuchMethodException
				| NullPointerException e) {
			FilePrinter.printError(FilePrinter.REACTOR + reactor.getId()
					+ ".txt", e);
		}
	}
			...
			try {
				try (PreparedStatement ps = DatabaseConnection
					...
					try (ResultSet rs = ps.executeQuery()) {
						while (rs.next()) {
							ret.add(new ReactorDropEntry(rs.getInt("itemid"),
									rs.getInt("chance"), rs.getInt("questid")));
						}
					}
				}
	...
	public void touch(MapleClient c, MapleReactor reactor) {
		this.touching(c, reactor, true);
	}

	public void untouch(MapleClient c, MapleReactor reactor) {
		this.touching(c, reactor, false);
	}
