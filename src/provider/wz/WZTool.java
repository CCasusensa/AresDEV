...
public class WZTool {
	...
	static {
		byte[] iv = new byte[] { (byte) 0x4d, (byte) 0x23, (byte) 0xc7,
				(byte) 0x2b, (byte) 0x4d, (byte) 0x23, (byte) 0xc7,
				(byte) 0x2b, (byte) 0x4d, (byte) 0x23, (byte) 0xc7,
				(byte) 0x2b, (byte) 0x4d, (byte) 0x23, (byte) 0xc7,
				(byte) 0x2b, };
		final byte[] key = new byte[] { (byte) 0x13, 0x00, 0x00, 0x00,
				(byte) 0x08, 0x00, 0x00, 0x00, (byte) 0x06, 0x00, 0x00, 0x00,
				(byte) 0xB4, 0x00, 0x00, 0x00, (byte) 0x1B, 0x00, 0x00, 0x00,
				(byte) 0x0F, 0x00, 0x00, 0x00, (byte) 0x33, 0x00, 0x00, 0x00,
	...
	public static String readDecodedString(LittleEndianAccessor llea) {
		...
		if (b >= 0) {
			...
			final byte str[] = new byte[strLength * 2];
			...
			final byte str[] = new byte[strLength];
	...
	public static String DecryptUnicodeStr(byte[] str) {
		int xorByte = 0xAAAA;
		final char[] charRet = new char[str.length / 2];
		for (int i = 0; i < str.length; i++) {
			str[i] = (byte) (str[i] ^ encKey[i]);
		}
		for (int i = 0; i < (str.length / 2); i++) {
			final char toXor = (char) ((str[i] << 8) | str[i + 1]);
	...
	public static String readDecodedStringAtOffsetAndReset(
		...
		slea.seek(offset);
		final String ret = readDecodedString(slea);
		slea.seek(pos);
		return ret;
	}
