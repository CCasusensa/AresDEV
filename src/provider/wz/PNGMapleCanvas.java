...
public class PNGMapleCanvas implements MapleCanvas {
	...
	public PNGMapleCanvas(int width, int height, int dataLength, int format,
			byte[] data) {
		super();
		this.height = height;
		this.width = width;
	...
	public BufferedImage getImage() {
		...
		final DataBufferByte imgData = new DataBufferByte(writeBuf,
				sizeUncompressed);
		final SampleModel sm = new PixelInterleavedSampleModel(
				DataBuffer.TYPE_BYTE, this.getWidth(), this.getHeight(), 4,
				this.getWidth() * 4, ZAHLEN);
		final WritableRaster imgRaster = Raster.createWritableRaster(sm,
				imgData, new Point(0, 0));
		final BufferedImage aa = new BufferedImage(this.getWidth(),
