...
public class WZIMGFile {
	...
	public WZIMGFile(File wzfile, WZFileEntry file, boolean provideImages,
			boolean modernImg) throws IOException {
		...
		this.parseExtended(this.root, slea, 0);
		this.root.finish();
		raf.close();
	}
	...
	private void parse(WZIMGEntry entry, SeekableLittleEndianAccessor slea) {
		...
		switch (marker) {
		case 0: {
			final String name = WZTool.readDecodedString(slea);
			entry.setName(name);
			break;
		}
		case 1: {
			final String name = WZTool.readDecodedStringAtOffsetAndReset(slea,
					this.file.getOffset() + slea.readInt());
			entry.setName(name);
			break;
		}
		default:
			System.out.println("Unknown Image identifier: " + marker
					+ " at offset "
					+ (slea.getPosition() - this.file.getOffset()));
		}
		...
		case 8:
			...
			if (iMarker == 0) {
				...
				entry.setData(WZTool.readDecodedStringAtOffsetAndReset(slea,
						slea.readInt() + this.file.getOffset()));
			} else {
		...
		case 9:
			...
			this.parseExtended(entry, slea, endOfExtendedBlock);
			break;
		default:
	...
	private void parseExtended(WZIMGEntry entry,
			SeekableLittleEndianAccessor slea, long endOfExtendedBlock) {
		byte marker = slea.readByte();
		String type;
		switch (marker) {
		case 0x73:
			...
			break;
		case 0x1B:
			type = WZTool.readDecodedStringAtOffsetAndReset(slea,
					this.file.getOffset() + slea.readInt());
			break;
		default:
			throw new RuntimeException("Unknown extended image identifier: "
					+ marker + " at offset "
					+ (slea.getPosition() - this.file.getOffset()));
		}
		if (type.equals("Property")) {
			...
			if (this.provideImages) {
				...
				entry.setData(new PNGMapleCanvas(width, height, dataLength,
						format + format2, pngdata));
			} else {
				entry.setData(new PNGMapleCanvas(width, height, dataLength,
						format + format2, null));
			...
			for (int i = 0; i < children; i++) {
				final WZIMGEntry cEntry = new WZIMGEntry(entry);
				this.parseExtended(cEntry, slea, 0);
				cEntry.finish();
				entry.addChild(cEntry);
			}
			...
			case 1:
				...
				break;
			default:
				System.out.println("Unknown UOL marker: " + uolmarker + " "
						+ entry.getName());
			}
		} else {
